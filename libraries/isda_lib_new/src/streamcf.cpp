...
***************************************************************************
*/
static int GetBackStub(TCurve *zeroCurve,    /* (I) Zero Curve          */
...
***************************************************************************
*/
static int GetFrontStub(TCurve *zeroCurve,    /* (I) Zero Curve        */
...
***************************************************************************
*/
static int CheckFloatingCFArgs(TDate valueDate, TDate indexStartDate,
                               TDate indexMatDate, long rateDayCountConv,
                               long payDayCountConv, double spread);
    ...
    char *holidayFile)    /* (I) Holiday file            */
{
  ...
  switch (stream->dl->stubLocation) {
    ...
    case JPMCDS_BACK_STUB:
      if (GetBackStub(zeroCurve, zcInterpType, stream, rateBadDayConv,
                      holidayFile,
    ...
    case JPMCDS_FRONT_STUB:
      if (GetFrontStub(zeroCurve, zcInterpType, stream, rateBadDayConv,
                       holidayFile, &amounts[0]) == FAILURE) {
  ...
  for (i = firstCoupon; i < lastCoupon; i++) {
    switch (stream->swapType) {
      case JPMCDS_SINGLE_REFIX:
        ...
        if (JpmcdsDtFwdAdj(indexStartDate, &stream->floatingRate->spotOffset,
                           &indexStartDate) == FAILURE) {
          JpmcdsErrMsg("%s: Error when finding index Date\n", routine);
          goto done;
        }

        if (JpmcdsDtFwdAny(indexStartDate, &stream->floatingRate->matInterval,
        ...
        if (JpmcdsBusinessDay(indexMatDate, rateBadDayConv, holidayFile,
                              &indexMatDate) == FAILURE) {
        ...
        if (TDATE_INTERVALS_EQUAL(stream->floatingRate->payInterval,
          ...
          if (JpmcdsMakeDateInterval(indexMatDate - indexStartDate, 'D',
                                     &payInterval) == FAILURE)
            goto done;
        } else {
        ...
        if (JpmcdsFloatingCashFlowGen(
                zeroCurve, zcInterpType, indexStartDate, indexMatDate,
                payInterval, &stream->dl->fArray[i],
                stream->floatingRate->dayCountConv, stream->payDayCountConv,
                stream->principal, stream->floatingRate->spread,
                &amounts[i]) == FAILURE) {
    ...
    TStreamFixed *stream) /* (I) See JpmcdsMakeTStream      */
{
  ...
  for (i = 0, j = cfl->fNumItems; i < stream->dl->fNumItems; i++, j++) {
    ...
    if (JpmcdsDayCountFraction(accStartDate, accEndDate,
                               stream->payDayCountConv, &yearFrac) == FAILURE) {
    ...
    cfl->fArray[j].fAmount = stream->principal * rate * yearFrac;
    ++cfl->fNumItems;

    if (i == stream->dl->fNumItems - 1 && stream->flags & JPMCDS_ADD_FINAL) {
    ...
    double *cashflow)          /* (O) Coupon payment              */
{
  ...
  if (CheckFloatingCFArgs(zeroCurve->fBaseDate, indexStartDate, indexMatDate,
                          rateDayCountConv, payDayCountConv,
                          spread) == FAILURE) {
  ...
  if (JpmcdsZerosToCouponsPoint(zeroCurve, zcInterpType, indexStartDate,
                                &payInterval, indexMatDate, rateDayCountConv,
                                FALSE, /* Stub at front */
                                &couponRate) == FAILURE) {
    goto done;
  }

  if (JpmcdsDayCountFraction(couponDates->accrueStartDate,
                             couponDates->accrueEndDate, payDayCountConv,
                             &yearFrac) == FAILURE) {
    goto done;
  }
...
static int CheckFloatingCFArgs(TDate valueDate, TDate indexStartDate,
  ...
  if (indexStartDate < valueDate) {
    JpmcdsErrMsg("%s: rate start (%s) is before value date (%s).\n", routine,
                 JpmcdsFormatDate(indexStartDate), JpmcdsFormatDate(valueDate));
    return FAILURE;
  }
    ...
    TFloatRate *floatingRate) /* (I) Defn of floating index  */
{
  ...
  if (stream->floatingRate == NULL) goto done;
                        ...
                        double *stubAmount)   /* (O) front stub rate   */
{
  ...
  if (GetFloatStubRate(zeroCurve, zcInterpType,
                       &stream->floatingRate->spotOffset, stream->averSoFar,
                       stream->compSoFar, stream->floatingRate->dayCountConv,
                       stream->dl->fArray[0], stream->swapType,
                       stream->firstCoupon.stubInterval, rateBadDayConv,
                       holidayFile, &stubRate) == FAILURE) {
  ...
  if (JpmcdsDayCountFraction(stream->dl->fArray[0].accrueStartDate,
  ...
  *stubAmount = stubRate * stream->principal * yearFrac;
                       ...
                       double *stubAmount)   /* (O) Back stub rate      */
{
  ...
  if (GetFloatStubRate(zeroCurve, zcInterpType,
                       ...
                       stream->swapType, stream->finalCoupon.stubInterval,
                       rateBadDayConv, holidayFile, &stubRate) == FAILURE) {
    ...
    double *stubRate)           /* (O) Rate for stub period */
{
  ...
  switch (swapType) {
    case JPMCDS_SINGLE_REFIX:
      if (JpmcdsForwardFromZCurve(zeroCurve, zcInterpType, startDate, matDate,
                                  dayCountConv, JPMCDS_SIMPLE_BASIS,
                                  &rate) == FAILURE) {
    ...
    TBoolean pricing)     /* (I) If pricing, no fixed rate expected*/
{
  ...
  if (pricing == FALSE &&
      (stream->fixedRate < -1.0 || stream->fixedRate > JPMCDS_MAX_RATE)) {
