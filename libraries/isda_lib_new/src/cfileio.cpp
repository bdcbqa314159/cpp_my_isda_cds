...
***************************************************************************
*/
TFile *JpmcdsFopen(char *filename, /* (I) Filename. */
...
***************************************************************************
*/
int JpmcdsFclose(TFile *tFile) /* (I) File handle. */
{
  static char routine[] = "JpmcdsFclose";
  int status = FAILURE; /* Until successful */

  if (tFile == NULL) return SUCCESS;

  if (fclose((FILE *)tFile->fileP) != 0) goto done;
...
***************************************************************************
*/
int JpmcdsFflush(TFile *tFile) /* (I) File handle. */
{
...
***************************************************************************
*/
int JpmcdsFgets(char *buffer, /* (I) Buffer to store the string. */
                ...
                TFile *tFile) /* (I) File handle. */
{
  ...
  if (fgets(tmpBuffer, numBytes, (FILE *)tFile->fileP) == NULL) return FAILURE;
...
***************************************************************************
*/
int JpmcdsFputs(char *output, /* (I) Strings which contain data. */
                TFile *tFile) /* (I) File handle. */
{
...
***************************************************************************
*/
int JpmcdsFread(char *ptr,    /* (I) Pointer to memory to store data. */
...
***************************************************************************
*/
int JpmcdsFwrite(char *ptr,    /* (I) Pointer to data to write. */
...
***************************************************************************
*/
int JpmcdsFgetc(TFile *tFile) /* (I) File handle. */
{
  int c;
  char tempChar;
  if (tFile->hasLastChar == TRUE) {
...
***************************************************************************
*/
int JpmcdsFputc(char c,       /* (I) Character to write. */
                TFile *tFile) /* (I) File handle. */
{
...
***************************************************************************
*/
int JpmcdsSprintf(char *buffer, /* (I) Buffer to write data to. */
...
***************************************************************************
*/
int JpmcdsSscanf(char *buffer, /* (I) File handle. */
                 char *fmt,    /* (I) Format string. */
                 ...)          /* (I) List of arguments for format string. */
{
  va_list args;
  ...
  if (numParse == EOF) return FAILURE; /* Note that EOF=FAILURE */
