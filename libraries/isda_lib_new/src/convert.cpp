...
***************************************************************************
*/
int JpmcdsStringToDate(char *mmddyyyy, /* (I) */
                       TDate *tdate)   /* (O) */
{
  char dateCopy[32];
  TMonthDayYear mdy;
  ...
  */
  (void)strncpy(dateCopy, mmddyyyy,
                sizeof(dateCopy) - 1); /* Don't alter original */
  ...
  if (strpbrk(dateCopy, "-/. ") == NULL) /* Check for delimiter chars */
  {
    ...
    if (strlen(dateCopy) != 8) goto done;
    cp = dateCopy;
    ...
    if (mdy.month == 0) goto done;

    /* Create the day. */
    smallBuf[0] = *cp++;
    smallBuf[1] = *cp++;
    mdy.day = atol(smallBuf);
    if (mdy.day == 0) goto done;
  } else /* Parse delimited format */
  {
    ...
    if (JpmcdsSscanf(cp, "%ld", &mdy.month) ==
        0) /* If not integer try string */
    {
      if (NStringToMonth(cp, &mdy.month) == FAILURE) goto done;
...
#define MAX_DIGITS 32
  ...
  while (isdigit((int)*inp)) *nump++ = *inp++;
