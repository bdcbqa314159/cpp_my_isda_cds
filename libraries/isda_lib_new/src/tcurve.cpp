...
***************************************************************************
*/
static int checkTCurve(TCurve *tc,     /* (I) TCurve to check */
    ...
    long dayCountConv) /* (I) */
{
  ...
  curve = JpmcdsMakeTCurveNoRateCheck(baseDate, dates, rates, numPts, basis,
                                      dayCountConv);
  if (curve == NULL) goto done;

  /* Make sure the TCurve is valid. This routine will guarantee we can
   * generate positive discount factors.
   */
  if (checkTCurve(curve, "TCurve", routine) != SUCCESS) goto done;
    ...
    long dayCountConv) /* (I) */
{
  ...
  /* Set up curve struct from Dates and Rates. */
  curve = JpmcdsNewTCurve(baseDate, numPts, basis, dayCountConv);
  ...
  for (idx = 0; idx < numPts; idx++) {
    ...
    if (idx > 0 && dates[idx - 1] >= dates[idx]) {
      JpmcdsErrMsg("%s: Date %d (%s) >= date %d (%s) in TCurve.\n", routine,
                   idx, JpmcdsFormatDate(dates[idx - 1]), idx + 1,
                   JpmcdsFormatDate(dates[idx]));
      goto done;
    } else {
...
***************************************************************************
*/
TDate *JpmcdsDatesFromCurve(TCurve *curve) /* (I) TCurve */
{
  ...
  if (dates == NULL) goto done;

  for (i = 0; i < curve->fNumItems; i++) dates[i] = curve->fArray[i].fDate;
...
***************************************************************************
*/
int JpmcdsCashFlowPV(TCashFlowList *cfl, /* (I) Cash Flow List */
                     ...
                     double *presValue)  /* (O) Present Value */
{
  ...
  for (idx = 0; idx < cfl->fNumItems; idx++) {
    if (JpmcdsDiscountDate(cfl->fArray[idx].fDate, zeroCurve, interpType,
                       ...
                       double *discount) /* (O) Present Value */
{
  ...
  if (JpmcdsInterpRate(date, zc, interpType, &interpRate) == FAILURE) goto done;

  if (JpmcdsDayCountFraction(zc->fBaseDate, date, zc->fDayCountConv,
                             &yearsToDate) == FAILURE)
    goto done;

  status = JpmcdsRateToDiscountYearFrac(interpRate, yearsToDate, zc->fBasis,
                                        discount);
...
TCurve *JpmcdsCopyCurve(TCurve *aCurve) /* (I) */
{
  ...
  newCurve = JpmcdsNewTCurve(aCurve->fBaseDate, aCurve->fNumItems,
                             aCurve->fBasis, aCurve->fDayCountConv);
  ...
  if (newCurve == NULL) JpmcdsErrMsg("%s: Failed.\n", routine);
                       ...
                       char *routine) /* (I) For error messages. */
{
  ...
  for (idx = 0; idx < tc->fNumItems; idx++) {
     ...
     */
    if (idx > 0 && tc->fArray[idx - 1].fDate >= tc->fArray[idx].fDate) {
      JpmcdsErrMsg("%s: Date %d (%s) >= date %d (%s) in %s.\n", routine, idx,
                   JpmcdsFormatDate(tc->fArray[idx - 1].fDate), idx + 1,
                   JpmcdsFormatDate(tc->fArray[idx].fDate), name);
      status = FAILURE;
    }
     ...
     * discount factors.
     */
    if (JpmcdsRateValid(routine, tc->fArray[idx].fRate, tc->fBaseDate,
                        tc->fArray[idx].fDate, tc->fDayCountConv,
                        tc->fBasis) != SUCCESS) {
      JpmcdsErrMsg(
          "%s: Rate %d (%f) at %s in %s implies "
