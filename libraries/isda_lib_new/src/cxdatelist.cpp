    ...
    TDate *array)  /* (I) [numItems] Dates to be added     */
{
  ...
  if (output == NULL) JpmcdsErrMsgFailure(routine);
    ...
    TStubMethod *stubType)   /* (I) Stub type */
{
  ...
  if (!stubType->stubAtEnd) {
    ...
    while (date > startDate) {
      ...
      SET_TDATE_INTERVAL(multiInterval, interval->prd * numIntervals,
                         interval->prd_typ);
      if (JpmcdsDtFwdAny(endDate, &multiInterval, &date) != SUCCESS) goto done;
    }
    assert(totalDates > 0);
    ...
    assert(totalDates > 0);
    assert(date >= endDate);
    if (date == endDate || totalDates == 1 ||
        stubType->stubAtEnd && !stubType->longStub) {
