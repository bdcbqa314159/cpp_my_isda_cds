...
***************************************************************************
*/
static int CheckZCCashInputs(TCurve *zeroCurve, TDate *dateArr, double *rateArr,
                             int numRates, long dayCountConv);
...
***************************************************************************
*/
static int CheckZCSwapsInputs(TCurve *zeroCurve, TCurve *discZC, TDate *dates,
                              double *rates, int numSwaps, int fixedSwapFreq,
                              int floatSwapFreq, long fixDayCountConv,
                              long floatDayCountConv, char fwdLength,
                     ...
                     long dayCountConv) /* (I) See JpmcdsDayCountConvention */
{
  ...
  if (CheckZCCashInputs(zeroCurve, dates, rates, numRates, dayCountConv) ==
      FAILURE)
    goto done;
  ...
  zcCash = JpmcdsZCMake(zeroCurve->fBaseDate, numRates + 2, ZC_DEFAULT_BASIS,
                        ZC_DEFAULT_DAYCNT);
  if (zcCash == NULL) {
  ...
  if (JpmcdsZCAddMoneyMarket(zcCash, dates, rates, numRates, dayCountConv) ==
      FAILURE) {
    ...
    char *holidayFile) /* (I) See JpmcdsBusinessDay              */
{
  ...
  /* Check inputs */
  if (CheckZCSwapsInputs(zeroCurve, discZC, dates, rates, numSwaps,
                         fixedSwapFreq, floatSwapFreq, fixDayCountConv,
                         floatDayCountConv, fwdLength, badDayConv,
                         holidayFile) == FAILURE) {
    goto done;
  }

  /* convert the input TCurve into a ZCurve so we can call all the usual zero
   * curve routines */
  ...
  if (numSwaps > 0) {
    if (JpmcdsZCAddSwaps(zcSwaps, discZC, &dates[offset], &rates[offset],
                         numSwaps, fixedSwapFreq, floatSwapFreq,
                         fixDayCountConv, floatDayCountConv,
                         JPMCDS_FLAT_FORWARDS, interpData, badDayList,
  ...
  for (i = 0; i < numRates; i++) {
    if (i > 0 && (dateArr[i - 1] > dateArr[i])) {
      JpmcdsErrMsg("%s: date #%d (%s) is after date #%d (%s).\n", routine, i,
                   JpmcdsFormatDate(dateArr[i - 1]), i + 1,
    ...
    if (dateArr[i] <= zeroCurve->fBaseDate) {
      JpmcdsErrMsg("%s: Input cash date (%s) on or before value date (%s).\n",
                   routine, JpmcdsFormatDate(dateArr[i]),
...
static int CheckZCSwapsInputs(TCurve *zeroCurve, TCurve *discZC, TDate *dates,
  ...
  for (i = 0; i < numSwaps - 1; i++) {
    if (dates[i + 1] <= dates[i]) {
      JpmcdsErrMsg("%s: date #%d (%s) is after date #%d (%s).\n", routine,
                   i + 1, JpmcdsFormatDate(dates[i]), i + 2,
                   JpmcdsFormatDate(dates[i + 1]));
      status = FAILURE;
    }
  }
   ...
   */
  if (JpmcdsIsBusinessDay(zeroCurve->fBaseDate, holidayFile, &isBusDay) ==
      FAILURE) {
