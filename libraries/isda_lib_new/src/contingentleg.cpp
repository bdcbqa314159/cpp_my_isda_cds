...
***************************************************************************
*/
static int onePeriodIntegral(TDate today, TDate startDate, TDate endDate,
                             TCurve *discountCurve, TCurve *spreadCurve,
                             double recoveryRate, double *pv);
...
***************************************************************************
*/
static int onePeriodIntegralAtPayDate(TDate today, TDate startDate,
                                      TDate endDate, TDate payDate,
                          ...
                          double *pv) /* (O) Present value of contingent leg */
{
  ...
  switch (cl->payType) {
    case PROT_PAY_MAT: {
      double tmp;
      if (onePeriodIntegralAtPayDate(today, startDate, cl->endDate, cl->endDate,
                                     discountCurve, spreadCurve, recoveryRate,
                                     &tmp) != SUCCESS)
        goto done;
    ...
    case PROT_PAY_DEF: {
      double tmp;
      if (onePeriodIntegral(today, startDate, cl->endDate, discountCurve,
                            spreadCurve, recoveryRate, &tmp) != SUCCESS)
        goto done;
...
static int onePeriodIntegral(TDate today, TDate startDate, TDate endDate,
  ...
  */
  ...
  for (i = 1; i < tl->fNumItems; ++i) {
     ...
     * noise on computer due to the small denominator. In this case, Talyor
     * expansion is employed to remove lambdafwdRate from denomintor so that
     * numerical noise is signicantly reduced.
     */
    if (fabs(lambdafwdRate) > 1.e-4) {
      thisPv = loss * lambda / lambdafwdRate * (1.0 - exp(-lambdafwdRate)) *
               s0 * df0;
    } else {
