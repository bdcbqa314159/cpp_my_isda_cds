...
***************************************************************************
*/
static int FeePaymentPVWithTimeLine(TAccrualPayConv accrualPayConv, TDate today,
                                    TDate stepinDate, TDate accStartDate,
                                    TDate accEndDate, TDate payDate,
                                    long accrueDCC, double notional,
                                    double couponRate, TCurve *discCurve,
                                    TCurve *spreadCurve, TDateList *tl,
                                    TBoolean obsStartOfDay, double *pv);
...
***************************************************************************
*/
static int FeeLegAI(TFeeLeg *fl, TDate today, double *ai);
...
int JpmcdsFeeLegPV(TFeeLeg *fl, TDate today, TDate stepinDate, TDate valueDate,
  ...
  matDate = (fl->obsStartOfDay == TRUE ? fl->accEndDates[fl->nbDates - 1] - 1
  ...
  for (i = 0; i < fl->nbDates; ++i) {
    ...
    if (FeePaymentPVWithTimeLine(fl->accrualPayConv, today, stepinDate,
                                 fl->accStartDates[i], fl->accEndDates[i],
                                 fl->payDates[i], fl->dcc, fl->notional,
                                 fl->couponRate, discCurve, spreadCurve, tl,
                                 fl->obsStartOfDay, &thisPv) != SUCCESS)
      goto done;
...
static int FeePaymentPVWithTimeLine(TAccrualPayConv accrualPayConv, TDate today,
  ...
  switch (accrualPayConv) {
    case ACCRUAL_PAY_NONE: {
      ...
      if (JpmcdsDayCountFraction(accStartDate, accEndDate, accrueDCC,
                                 &accTime) != SUCCESS)
        goto done;

      amount = notional * couponRate * accTime;
      survival =
          JpmcdsForwardZeroPrice(spreadCurve, today, accEndDate + obsOffset);
    ...
    case ACCRUAL_PAY_ALL: {
      ...
      if (JpmcdsAccrualOnDefaultPVWithTimeLine(
              today, stepinDate + obsOffset, accStartDate + obsOffset,
              accEndDate + obsOffset, amount, discCurve, spreadCurve, tl,
              &accrual) != SUCCESS)
        goto done;
...
int JpmcdsAccrualOnDefaultPVWithTimeLine(TDate today, TDate stepinDate,
                                         ...
                                         TCurve *spreadCurve,
                                         TDateList *criticalDates, double *pv) {
  ...
  */
  ...
  for (i = 1; i < tl->fNumItems; ++i) {
    ...
    /**
     * If lambdafwdRate is extremely small, original calculation generates big
     * noise on computer due to the small denominators. In this case, Talyor
     * expansion is employed to remove lambdafwdRate from denomintors so that
     * numerical noise is signicantly reduced.
     */
    if (fabs(lambdafwdRate) > 1e-4) {
      /*This is the original formula which contains an integral*/
      thisPv =
          lambda * accRate * s0 * df0 *
      ...
      */
      ...
      const double lambdaAccRateLamdaFwdRate =
          lambdaAccRate * lambdafwdRate / 3.;
      const double thisPv2 = -lambdaAccRateLamdaFwdRate * (t0 + 2. * t1);

      const double lambdaAccRateLamdaFwdRate2 =
          lambdaAccRateLamdaFwdRate * lambdafwdRate * .25;
      const double thisPv3 = lambdaAccRateLamdaFwdRate2 * (t0 + 3. * t1);

      const double lambdaAccRateLamdaFwdRate3 =
          lambdaAccRateLamdaFwdRate2 * lambdafwdRate * .2;
      const double thisPv4 = -lambdaAccRateLamdaFwdRate3 * (t0 + 4. * t1);

      const double lambdaAccRateLamdaFwdRate4 =
          lambdaAccRateLamdaFwdRate3 * lambdafwdRate / 6.;
      ...
      const double lambdaAccRateLamdaFwdRate2 = lambdaAccRateLamdaFwdRate *
      lambdafwdRate; const double thisPv3 = lambdaAccRateLamdaFwdRate2 * .125;

      const double lambdaAccRateLamdaFwdRate3 = lambdaAccRateLamdaFwdRate2 *
      lambdafwdRate; const double thisPv4 = -lambdaAccRateLamdaFwdRate3 / 30.;

      const double lambdaAccRateLamdaFwdRate4 = lambdaAccRateLamdaFwdRate3 *
      lambdafwdRate; const double thisPv5 = lambdaAccRateLamdaFwdRate4 / 144.;
      */
...
TCashFlowList *JpmcdsFeeLegFlows(TFeeLeg *fl) {
  ...
  for (i = 0; i < fl->nbDates; ++i) {
    ...
    if (JpmcdsDayCountFraction(fl->accStartDates[i], fl->accEndDates[i],
                               fl->dcc, &time) != SUCCESS) {
      goto done;
    }
...
static int FeeLegAI(TFeeLeg *fl, TDate today, double *ai) {
  ...
  if (JpmcdsBinarySearchLong(today, fl->accStartDates, sizeof(TDate),
                             fl->nbDates, &exact, &lo, &hi) != SUCCESS)
    goto done;
  ...
  if (exact >= 0) {
    ...
    *ai = 0;
    ...
    double accrual;
    if (JpmcdsDayCountFraction(fl->accStartDates[lo], today, fl->dcc,
                               &accrual) != SUCCESS)
      goto done;
