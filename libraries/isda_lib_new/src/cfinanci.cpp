    ...
    double *rate)          /* (O) output rate */
{
  ...
  if (rateBasis == JPMCDS_DISCOUNT_FACTOR) {
    if (startDate == endDate) {
      ...
      if (!ARE_ALMOST_EQUAL(discount, 1.0)) {
        JpmcdsErrMsg(
            "%s: Start date (%s) equals end date (%s), "
            "but input disount factor (%f) <> 1.0\n",
            routine, JpmcdsFormatDate(startDate), JpmcdsFormatDate(endDate),
            discount);
        goto done;
      }
      *rate = 1.0;
  ...
  if (JpmcdsDayCountFraction(startDate, endDate, rateDayCountConv, &rateYF) ==
      FAILURE) {
    goto done;
  }

  if (JpmcdsDiscountToRateYearFrac(discount, rateYF, (double)rateBasis, rate) !=
      SUCCESS) {
    goto done;
  }
    ...
    double *discount)      /* (O) Discount factor */
{
  ...
  if (JpmcdsRateToDiscountYearFrac(rate, rateYF, (double)rateBasis, discount) !=
      SUCCESS) {
    goto done;
  }
...
***************************************************************************
*/
int JpmcdsRateValid(char *routine,         /* (I) Routine name to print */
                    ...
                    double rateBasis)      /* (I) Compounding basis */
{
  ...
  switch ((long)rateBasis) {
    ...
    case JPMCDS_DISCOUNT_RATE:
       ...
       */
      if (JpmcdsDayCountFraction(startDate, endDate, rateDayCountConv,
  ...
  if (JpmcdsRateValidYearFrac(routine, rate, yearFraction, rateBasis) !=
      SUCCESS) {
    goto done;
  }
...
***************************************************************************
*/
int JpmcdsRateValidYearFrac(char *routine,       /* (I) Routine name to print */
                            double rate,         /* (I) Rate to validate */
  ...
  switch ((long)basis) {
    case JPMCDS_SIMPLE_BASIS:
      if (rate * yearFraction <= -1.0) {
        JpmcdsErrMsg(
            "%s: Simple Rate (%f) * Year Fraction (%f) must "
            "be > -1.0.\n",
    ...
    case JPMCDS_DISCOUNT_RATE:
      if (rate * yearFraction >= 1.0) {
        JpmcdsErrMsg(
            "%s: Discount Rate (%f) * Year Fraction (%f) must "
            "be < 1.0.\n",
            routine, rate, yearFraction);
        goto done;
      }
