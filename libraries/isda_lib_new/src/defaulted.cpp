...
EXPORT int JpmcdsDefaultAccrual(TDate tradeDate, TDate edd, TDate startDate,
  ...
  *defaultAccrual = 0.;
  ...
  if (edd < startDate) goto done;

  fl = JpmcdsCdsFeeLegMake(startDate, endDate, TRUE, couponInterval, stubType,
                           notional, couponRate, paymentDcc, badDayConv,
                           calendar, TRUE);

  if (NULL == fl) goto done;

  i = 0;
  while (i < fl->nbDates) {
    if (fl->accStartDates[i] <= tradeDate && tradeDate < fl->accEndDates[i]) {
      *accrualDays = edd + 1.0 - fl->accStartDates[i];
      if (JpmcdsDayCountFraction(fl->accStartDates[i], edd + 1, paymentDcc,
