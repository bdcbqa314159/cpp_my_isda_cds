...
***************************************************************************
*/
static TBoolean checkDateInterval(TDateInterval *ptrDateInterval,
                                  TDate fromDate, TDate toDate, char *caller);
...
#define YEAR_HAS_A_LEAP_DAY(y) \
  (EXACTLY_DIVIDES(y, 400) ||  \
                     ...
                     int *extraDays) /* (O) Days left over(remainder) */
{
   ...
   */
  if (JpmcdsDateIntervalToYears(interval, &intervalYears) == FAILURE) goto done;
...
***************************************************************************
*/
int JpmcdsFreq2TDateInterval(long freq,               /* (I) */
  ...
  switch (toupper(interval->prd_typ)) {
    ...
    case 'T':
      *years = (double)(interval->prd) / JPMCDS_MONTHS_PER_YEAR; /* 30/360 */
      break;
    case 'W':
      *years = (double)interval->prd * (double)JPMCDS_DAYS_PER_WEEK /
...
***************************************************************************
*/
int JpmcdsDateFromDateAndOffset(TDate oldDate, TDateInterval *pInterval,
                                int index, TDate *pNewDate) {
