...
***************************************************************************
*/
static int JpmcdsCalcCoupon(TCurve *zc, long interpType, TCashFlowList *cfl,
                            double presentValue, double *couponRate);
    ...
    double *couponRate)      /* (O) Rate for this instrument */
{
  ...
  if (JpmcdsZerosToCouponsPointAdj(zc, interpType, startDate, interval,
                                   maturityDate, fixedDCC, stubAtEnd,
                                   JPMCDS_BAD_DAY_NONE, JPMCDS_BAD_DAY_NONE,
                                   "NO_WEEKENDS", couponRate) == FAILURE)
    goto done;
    ...
    double *couponRate)      /* (O) Rate for this instrument */
{
  ...
  if (JpmcdsBusinessDay(startDate, accBadDayConv, holidayFile, &startDateAdj) ==
      FAILURE)
    goto done;

  if (JpmcdsCurveFirstDate(zc, &zcFirstDate) != SUCCESS) goto done;
  ...
  if (startDateAdj < zcFirstDate) {
    ...
    if (accBadDayConv != JPMCDS_BAD_DAY_NONE && startDate != startDateAdj) {
      JpmcdsErrMsg("%s: Bad day adjustment moved start date from %s to %s.\n",
   ...
   */
  cfl =
      JpmcdsMakeCFL(1. /* coupon*/, startDate, interval, maturityDate, fixedDCC,
                    stubAtEnd, 0 /* No initial payment, no final repayment */,
                    accBadDayConv, payBadDayConv, holidayFile);

  if (cfl == (TCashFlowList *)NULL) goto done;
...
***************************************************************************
*/
static int JpmcdsCalcCoupon(TCurve *zc,         /* (I) Zero-coupon rate curve */
                            ...
                            double *couponRate)  /* (O) Annual coupon rate */
{
  ...
  if (JpmcdsCashFlowPV(cfl, zc, interpType, &coupons_PV) == FAILURE) goto done;
  ...
  if (JpmcdsDiscountDate(cfl->fArray[num_pts - 1].fDate, zc, interpType,
                         &last_PV) == FAILURE)
    goto done;
