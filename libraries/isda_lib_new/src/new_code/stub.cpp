...
***************************************************************************
*/
int JpmcdsStubPayment(TDate prevCouponDate, /* (I) Needed for Actual/Actual */
                      ...
                      double *stubPayment)     /* (O) At accrue End date */
{
  ...
  if (prevCouponDate >= nextCouponDate) {
    Jpmcds::ErrMsg("%s: prevCouponDate (%s) >= nextCouponDate (%s).\n", routine,
  ...
  if (stubStart < prevCouponDate || stubStart > nextCouponDate) {
    Jpmcds::ErrMsg(
        ...
        routine, JpmcdsFormatDate(stubStart), JpmcdsFormatDate(prevCouponDate),
        JpmcdsFormatDate(nextCouponDate));
    return FAILURE;
  }
  ...
  if (stubEnd > nextCouponDate || stubEnd < prevCouponDate) {
    Jpmcds::ErrMsg(
        ...
        routine, JpmcdsFormatDate(stubEnd), JpmcdsFormatDate(nextCouponDate),
        JpmcdsFormatDate(prevCouponDate));
    return FAILURE;
  }
  ...
  if (stubStart != prevCouponDate && stubEnd != nextCouponDate) {
    Jpmcds::ErrMsg(
        "%s: StubStart(%s) <> prevCouponDate(%s) AND"
        " StubEnd(%s) <> nextCouponDate(%s).\n",
        routine, JpmcdsFormatDate(stubStart), JpmcdsFormatDate(prevCouponDate),
        JpmcdsFormatDate(stubEnd), JpmcdsFormatDate(nextCouponDate));
    return FAILURE;
  }

  /* Compute coupon year fraction and coupon.
   */
  if (JpmcdsDayCountFraction(prevCouponDate, nextCouponDate, couponDayCountConv,
                             &couponYearFrac) == FAILURE)
    return FAILURE;
  coupon = rate * couponYearFrac;

  /* Compute stub year fraction and stub fraction. */
  if (JpmcdsDayCountFraction(stubStart, stubEnd, couponDayCountConv,
                             &stubYears) == FAILURE)
    return FAILURE;
...
int JpmcdsStringToStubMethod(
  ...
  static char invalidFormatMsg[] =
      "%s: Invalid stub method format %s. "
  ...
  if (JpmcdsStringParser(buf, "/", &lengthStr) != SUCCESS) goto done;
  ...
  switch (toupper(*locStr)) {
    ...
    case 'B':
      ...
      break;
    default:
      Jpmcds::ErrMsg(invalidFormatMsg, routine, name);
      goto done;
  }
  ...
  if (lengthStr == NULL) {
    ...
    switch (toupper(*lengthStr)) {
      ...
      case 'L':
        ...
        break;
      default:
        Jpmcds::ErrMsg(invalidFormatMsg, routine);
        goto done;
    }
  }
...
*********************************************************************
*/
int JpmcdsIsEndStub(TDate startDate,     /* (I) Start date */
                    ...
                    TBoolean *isEndStub) /* (O) TRUE if end stub */
{
  ...
  switch (stubPos) {
    ...
    case JPMCDS_STUB_POS_DEFAULT_AUTO:
      if (JpmcdsCountDates(startDate, maturityDate, ivl, &numIvls,
                           &extraDays) != SUCCESS) {
        goto done;
      }
  ...
  switch (stubPosValue) {
    ...
    case JPMCDS_STUB_POS_DEFAULT_BACK:
      *stubPos = JPMCDS_STUB_POS_DEFAULT_BACK;
      break;
    default:
      Jpmcds::ErrMsg("%s: Unknown stub pos %ld.\n", routine, stubPosValue);
      goto done;
  }
