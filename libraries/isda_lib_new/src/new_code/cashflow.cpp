...
***************************************************************************
*/
TCashFlowList *JpmcdsMergeCFL(TCashFlowList *a, /* (I) first cash flow list */
                              TCashFlowList *b) /* (I) second cash flow list */
{
  ...
  while (ia < a->fNumItems && ib < b->fNumItems) {
    ...
    if (aDate < bDate) {
      ...
      cfl->fArray[i++].fAmount =
          a->fArray[ia++].fAmount + b->fArray[ib++].fAmount;
    }
  }
  ...
  if (cfl == NULL) Jpmcds::ErrMsg("%s: FAILED\n", routine);
...
TCashFlowList *JpmcdsNewEmptyCFL(int numItems) {
  ...
  if (numItems < 0) {
    Jpmcds::ErrMsg("%s: # cashflows (%d) must be >= 0.\n", routine, numItems);
    goto done;
  }
  ...
  if (numItems > 0) {
    cfl->fArray = NewArray<TCashFlow>(numItems);
    if (cfl->fArray == NULL) goto done;
  } else {
    cfl->fArray = NULL;
  }
  /* Success
    ...
    char *holidayFile)       /* (I) Holiday file */
{
  ...
  if (maturityDate <= startDate) {
    Jpmcds::ErrMsg("%s: maturityDate(%s) <= startDate(%s)\n", routine,
                   JpmcdsFormatDate(maturityDate), JpmcdsFormatDate(startDate));
    return NULL;
  }

  if ((flags & JPMCDS_SUBTRACT_INITIAL || flags & JPMCDS_KEEP_START_DATE) &&
      flags & JPMCDS_PRESTART_ZERO_PAYMENT) {
    Jpmcds::ErrMsg("%s: cannot subtract initial payment at prestart date.\n",
                   routine);
    Jpmcds::ErrMsg("\tAlso cannot keep both the start and prestart dates.\n");
    return NULL;
  }

  if (flags & JPMCDS_POST_MATURITY_ZERO_PAYMENT) {
    if (stubPlacement & JPMCDS_STUB_POSN_MASK) {
      Jpmcds::ErrMsg("%s: no post maturity zero payment w/ stub at end.\n",
                     routine);
      return NULL;
    } else if (flags & JPMCDS_ADD_FINAL) {
      Jpmcds::ErrMsg("%s: no post maturity zero payment w/ final payment.\n",
                     routine);
      return NULL;
   ...
   */
  dl = JpmcdsNewDateListExtendedRoll(startDate, maturityDate,
                                     startDate /* roll date */, interval,
                                     stubPlacement & JPMCDS_STUB_POSN_MASK);
  if (dl == (TDateList *)NULL) goto done;

  /* Make cash flow list based on an annual rate of couponRate.
   * The startDate is included, with a cashflow of 0.
   */
  cfl = JpmcdsNewCFLFromDL3(
      dl, startDate, maturityDate, couponRate, couponDayCountConv,
      stubPlacement & JPMCDS_STUB_LENGTH_LONG, accrualBadDayConv, payBadDayConv,
   ...
   */
  if (!(flags & JPMCDS_PRESTART_ZERO_PAYMENT ||
    ...
    TBoolean usePreStartDate) /* (I) Use prestart date if stub */
{
  ...
  if (longStub && (frontStub || backStub) && numDates > 2) {
    ...
    if (datesCopy == NULL) goto done;

    if (frontStub) {
      datesCopy->fArray[0] = dates->fArray[0];
      memcpy(&datesCopy->fArray[1], &dates->fArray[2],
             (numDates - 2) * sizeof(TDate));
      dates = datesCopy; /* simply overwrite pointer, hiding original */
      --numDates;
    }

    if (backStub) {
      memcpy(&datesCopy->fArray[0], &dates->fArray[0],
             (numDates - 2) * sizeof(TDate));
  ...
  if (accrualBadDayConv == JPMCDS_BAD_DAY_NONE) {
    ...
    accrualDates = JpmcdsNewDateListBusDayAdj(dates, accrualBadDayConv,
                                              accrualHolidayFile);
    if (accrualDates == NULL) goto done;
  ...
  for (idx = startIdx; idx <= endIdx; idx++) {
    ...
    if (JpmcdsDayCountFraction(accrualDates->fArray[idx - 1],
                               accrualDates->fArray[idx], couponDayCountConv,
                               &yearFrac) == FAILURE)
      goto done;
  ...
  if (payDates != dates) JpmcdsFreeDateList(payDates);

  if (accrualDates != dates) JpmcdsFreeDateList(accrualDates);
