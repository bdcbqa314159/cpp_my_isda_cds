...
TDateList *JpmcdsNewEmptyDateList(int numDates) {
  ...
  if (numDates > 0) {
    dateList->fArray = NewArray<TDate>(numDates);
    if (dateList->fArray == NULL) goto done;
  } else /* numItems=0 */
  {
...
int JpmcdsExtendDateList(TDateList *dateList, /* (I/O) Date list to extend. */
  ...
  if (numDates > dateList->fNumItems) {
    TDate *newArray = NewArray<TDate>(numDates);
    if (newArray == NULL) goto done;

    CopyArray<TDate>(newArray, dateList->fArray, dateList->fNumItems);
    FreeArray(dateList->fArray);
...
***************************************************************************
*/
TDateList *JpmcdsCopyDateList(TDateList *a) /* (I) Date list to copy */
{
...
TDateList *JpmcdsSortDateList(TDateList *dl) /* (I) */
{
  ...
  sortedDL = JpmcdsNewDateListFromDates(dl->fArray, dl->fNumItems);
  if (sortedDL == NULL) goto done;

  qsort(sortedDL->fArray, (size_t)sortedDL->fNumItems, (size_t)sizeof(TDate),
        compareTDatesFcn);
    ...
    TBoolean allowDuplicates) /* (I) TRUE if duplicates in merged list */
{
  ...
  if (dateList1 == NULL) {
    ...
    outputDates = JpmcdsJpmcdsDateListAddDates(
        dateList1, dateList2->fNumItems, dateList2->fArray, allowDuplicates);
    if (outputDates == (TDateList *)NULL) {
      goto done;
    }
  }
...
TDateList *JpmcdsJpmcdsDateListAddDates(
  ...
  for (indxOut = 0; indx1 < dateList->fNumItems && indx2 < numDates;
       indxOut++) {
...
TDateList *JpmcdsSubtractDateLists(
  ...
  for (indx1 = indxOut = 0; indx1 < dateList->fNumItems; indx1++) {
    for (indx2 = 0; indx2 < subDates->fNumItems; indx2++) {
      ...
      if (dateList->fArray[indx1] == subDates->fArray[indx2]) break;
  ...
  /* free up memory */
  JpmcdsFreeDateList(unsortedDates);
  unsortedDates = NULL;
  ...
  if (status != SUCCESS) {
    JpmcdsFreeDateList(unsortedDates);
    unsortedDates = NULL;
    JpmcdsFreeDateList(sortedDates);
    sortedDates = NULL;
    JpmcdsErrMsgFailure(routine);
  }
  return sortedDates;
...
TDateList *JpmcdsNewDateListDistinct(
  ...
  if (distinctDL == (TDateList *)NULL) goto done;
    ...
    TBoolean inPlace)       /* (I) TRUE=modify date list in place */
{
  ...
  if (truncated == NULL) JpmcdsErrMsgFailure(routine);
