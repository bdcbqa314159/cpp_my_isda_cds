    ...
    TBoolean *isBusinessDay) /* (O) TRUE if a business day */
{
  ...
  if (JpmcdsIsWeekend(date, hl->weekends)) {
    *isBusinessDay = FALSE;
    return SUCCESS;
  }
  ...
  */
  ...
  do { /* Check if it's a holiday first. */
    if (!doneSearchingList && curDate == holArray[holIdx]) {
      ...
      if (JpmcdsIsWeekend(curDate, hl->weekends)) {
        curDate += direction;
      } else {
        aHoliday = FALSE;
      }
  ...
  switch (hl->weekends) {
    ...
    default:
      ...
      while (ABS(curDate - toDate) > 0) {
        curDate += signum;
        if (JpmcdsIsWeekday(curDate, hl->weekends)) {
          nrExtraDays++;
        }
      }
  ...
  while (numBusDaysLeft > 0) {
    ...
    */
    ...
    } else /* Not a holiday. */
    {
      ...
      */
      if (JpmcdsIsWeekday(curDate, weekends)) {
        numBusDaysLeft--;
      }
    }
  }
