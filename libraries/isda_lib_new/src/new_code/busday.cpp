...
#define IS_EMPTY_DATELIST(pdatelist) ((pdatelist)->fNumItems > 0 ? FALSE : TRUE)
...
#endif
#define INITIAL_ARRAY_SIZE                                                     \
  2000 /* starting size of array for holding dates for a single holiday centre \
        */
...
static int getNextBusDate(TDate startDate,  /* (I) starting date. */
                          long direction,   /* (I) +1=forwards, -1=backwards */
...
static int readHolidaysAll(
    ...
    const TBoolean addSupplementaryDates /* (I) supplemantary dates if any are
                                            add (TRUE) or subtract (FALSE) */
);

static int stringToDate(const char *str, /* (I) string to convert */
                        long *date       /* (O) date              */
);
    ...
    char *filename) /* (I) filename to load */
{
  ...
  if (JpmcdsHolidayListAddToCache(name, hl) != SUCCESS) goto done;
    ...
    TBoolean *isBusinessDay) /* (O) TRUE if a business day */
{
  ...
  if (hl == NULL) {
    // TODO work here
    std::cout << "Error: " << routine << ": hl is NULL.\n";
    ...
    TBoolean *isHoliday) /* (O) 0 = not a holiday, 1 = is a holiday */
{
  ...
  *isHoliday = FALSE;
  ...
  if (dates == NULL) {
    ...
    // TODO work here
    //  PROGRAM_BUG();
    std::cout << "Error: " << routine << ": hl->dateList->fArray is NULL.\n";
    return FAILURE;
  }
  ...
  ** is in the list.
  */
  bRet = (TDate *)bsearch((void *)&date, (void *)&dates[0],
                          (size_t)hl->dateList->fNumItems,
                          (size_t)sizeof(TDate), holidayCompare);
...

{
  ...
  // TODO work here
  //    if (status != SUCCESS) JpmcdsErrMsg("%s: Failed.\n", routine);
  if (status != SUCCESS) std::cout << "Error: " << routine << ": Failed.\n";
...

{
  ...
  *outDate = date;
  ...
  switch (badDayConv) {
    ...
    case JPMCDS_BAD_DAY_FOLLOW:
      intervalSign = +1;
      if (getNextBusDateMulti(date, intervalSign, numHolidayLists, holidayLists,
                              &nextDate) != SUCCESS)
        goto done;
      break;
    ...
    case JPMCDS_BAD_DAY_MODIFIED:
      ...
      */
      ...
      if (JpmcdsDateToMDY(nextDate, &mdy1) != SUCCESS) goto done;

      if (JpmcdsDateToMDY(date, &mdy2) != SUCCESS) goto done;

      if (mdy1.month != mdy2.month) {
        /* Go backwards. */
        if (getNextBusDateMulti(date, -intervalSign, numHolidayLists,
                                holidayLists, &nextDate) != SUCCESS)
          goto done;
      }
      break;
    ...
    default:
      ...
      //    badDayConv);
      std::cout << "Error: " << routine
                << ": Unrecognized bad day convention = " << badDayConv
                << ".\n";
      goto done;
  }
...
static int getNextBusDateMulti(
  ...
  *nextDate = startDate;
  ...
  /* this loop executes getNextBusDate for each market in sequence; whenever
     weekends/ holidays force an adjustment, the process recommences at the
     first market; eventually, a date will be reached which is a business day in
     all markets, whereupon the loop will execute to completion

     we can be certain that all of the skipped dates were weekends/holidays in
     at least one of the markets, i.e. output will be the _first_ suitable day
   */

  n = 0;
  while (n < numHolidayLists) {
    if (getNextBusDate(*nextDate, direction, holidayLists[n], &adjDate) !=
        SUCCESS) {
      goto done;
    }
...
static int getNextBusDate(TDate startDate,  /* (I) starting date. */
  ...
  if (IS_EMPTY_DATELIST(hl->dateList)) {
    ...
    if (findFirstHolidayIdx(startDate, hl->dateList, direction, &holIdx,
      ...
      std::cout << "Error: getNextBusDate: Failed.\n";
      return FAILURE;
    }
  }
  ...
  */
  ...
  do { /* Check if it's a holiday first. */
    if (!doneSearchingList && curDate == holArray[holIdx]) {
      holIdx += direction;
      curDate += direction;
      doneSearchingList = (TBoolean)(holIdx < 0 || holIdx >= numHols);
    } else {
    ...
    long *result)     /* (O) Result value       */
{
  ...
  *result = 0L; /* TDate EQ long */
  ...
  if (fromDate == toDate) return SUCCESS;
  ...
  */
  ...
  if (!IS_EMPTY_DATELIST(hl->dateList)) {
    ...
    if (findFirstHolidayIdx(fromDate + signum, hl->dateList, signum, &holIdx,
                            &doneSearching) != SUCCESS)
      return FAILURE;

    if (!doneSearching) {
      numHolidays = calcNumWeekdayHolidays(toDate, holIdx, signum, hl->weekends,
                                           hl->dateList, &holIdx);
    }
  }
    ...
    if (numBusDaysLeft == 0) {
      ...
      *resultDate = fromDate + 7 * (numBusDaysLeft / 5) +
      ...
      if (hl->dateList->fNumItems > 0) {
        if (holAdjustStandardWeekends(fromDate, hl->dateList, resultDate) !=
            SUCCESS)
          goto done;
      }
    }
  } else {
    if (forwardNonStandardWeekends(fromDate, numBusDaysLeft, intervalSign,
                                   hl->weekends, busDaysPerWeek, hl->dateList,
                                   resultDate) != SUCCESS)
      goto done;
  }
...
static int forwardNonStandardWeekends(
  ...
  */
  ...
  if (numHols > 0) {
    ...
    ** order.
    */
    if (findFirstHolidayIdx(fromDate + direction, hlDatelist, direction,
                            &holIdx, &doneSearchingList) != SUCCESS) {
      goto done;
    }
  } else {
  ...
  */
  if (!doneSearchingList) {
    numHolidays = calcNumWeekdayHolidays(curDate, holIdx, direction, weekends,
                                         hlDatelist, &holIdx);
    doneSearchingList = (TBoolean)(holIdx < 0 || holIdx >= numHols);
  } else {
    numHolidays = 0;
...
static int holAdjustStandardWeekends(
  ...
  */
  if (numHols <= 0 || fromDate >= holDates[numHols - 1]) {
    ...
    if (fromDate <= holDates[0]) {
      ...
      if (JpmcdsBSearchLong((double)fromDate, holDates, sizeof(TDate), numHols,
                            &idxLo, &i) == FAILURE) {
        goto done;
      }
...
static long calcNumWeekdayHolidays(
  ...
  {
    for (k = startHolIdx; (k < numItems && dateArray[k] <= toDate); k++) {
      numHols++;
    }
    *endHolIdx = k;
    if (k < numItems && dateArray[k] == toDate) {
      (*endHolIdx)++;
    }
  } else /* backward */
  {
    for (k = startHolIdx; (k >= 0 && dateArray[k] >= toDate); k--) {
      numHols++;
    }
    *endHolIdx = k;
    if (k >= 0 && dateArray[k] == toDate) {
      (*endHolIdx)--;
    }
  }
...
int JpmcdsHolidayListDateToBusinessEOM(
  ...
  */
  ...
  if (JpmcdsDateToEOM(inDate, outDate) != SUCCESS) goto done;

  if (JpmcdsHolidayListBusinessDay(*outDate, JPMCDS_BAD_DAY_PREVIOUS, hl,
                                   outDate) != SUCCESS)
    goto done;
