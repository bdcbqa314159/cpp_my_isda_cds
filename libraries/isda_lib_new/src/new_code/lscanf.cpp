...
static double exptable[] = {
    1.0e-30, 1.0e-29, 1.0e-28, 1.0e-27, 1.0e-26, 1.0e-25, 1.0e-24, 1.0e-23,
    ...
    1.0e18,  1.0e19,  1.0e20,  1.0e21,  1.0e22,  1.0e23,  1.0e24,  1.0e25,
...
static int localIsspace(int c) { return isspace(c); }
...
int JpmcdsLvfscanf(TFile *tFile, char *fmt, va_list arg) {
  ...
  conv:
    switch (c) {
      ...
      case 'c':
        if (!F.haveWidth) F.fieldWidth = 1;
        if (!F.suppress) s = va_arg(arg, char *);
        while (F.fieldWidth-- > 0) {
          if ((c = localGetc(tFile)) == EOF) goto done;
          if (!F.suppress) *s++ = (char)c;
          ++nread;
        }
        if (!F.suppress) ++nassigned;
        ++nconverted;
        continue;
  ...
  string:
    if (!F.haveWidth) F.fieldWidth = INT_MAX;
    if (!F.suppress) s = va_arg(arg, char *);
    for (; c != EOF; c = localGetc(tFile), ++nread) {
      ...
      F.valid = TRUE;
      if (!F.suppress) *s++ = (char)c;
      if (F.fieldWidth == 0) goto endstring;
    }
    localUngetc(tFile, c), --nread;
  endstring:
    if (!F.valid) goto done;
    if (!F.suppress) {
      *s = 0;
      ++nassigned;
    }
  ...
  conv_unsigned:
    F.unsigned_ = TRUE;
  conv_signed:
    if (F.fetchBase) base = (short)va_arg(arg, int);
    state = 0;
  ...
  scan:
    ...
    for (; c != EOF; c = localGetc(tFile), ++nread) {
      ...
      if (F.fieldWidth == 0) goto endscan;
    }
  ...
  endscan:
    ...
    if (F.negate && result) {
      result = -result;
      if (F.unsigned_ || result > 0) overflow = TRUE;
    ...
    if (F.hSize) {
      if (F.unsigned_) {
        if ((long)(unsigned short)result != result) overflow = TRUE;
      } else {
        if ((short)result != result) overflow = TRUE;
      }
    } else if (!F.lSize) {
      if (F.unsigned_) {
        if ((long)(unsigned int)result != result) overflow = TRUE;
      } else {
        if ((int)result != result) overflow = TRUE;
      }
    }
    ...
    if (overflow) {
      if (F.unsigned_)
        ...
        result = INT_MIN;
      if (!F.negate) result = ~result;
      if (!F.floating) errno = ERANGE;
    }
  ...
  if (nconverted == 0 && c == EOF) return EOF;
 ...
 */
static void ldtof(short code, struct decrec *d, void *p) {
