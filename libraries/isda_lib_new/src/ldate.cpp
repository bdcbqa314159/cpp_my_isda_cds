...
***************************************************************************
*/
int JpmcdsIsLeap(TDate DATE,   /* (I) DATE */
  ...
  *IS_LEAP = 0; /* assume not a leap year */
  ...
  if (JpmcdsDateToMDY(DATE, &temp) == FAILURE) return FAILURE;
...
***************************************************************************
*/
int JpmcdsDtFwdAny(TDate startDate,         /* (I) date */
  ...
  switch (upperPrdTyp) {
    ...
    case 'Q': /* QUARTERly increments */
      ...
      if (dateToMDYFast(startDate, &mdy) == FAILURE) goto done;

      mdy.month += intval.prd;
      if (JpmcdsNormalizeMDY(&mdy) == FAILURE) goto done;

      if (JpmcdsMDYToDate(&mdy, sumDate) == FAILURE) goto done;
      break;
...
***************************************************************************
*/
int JpmcdsDayCountFraction(TDate date1,   /* (I) Start date */
   ...
   */
  if (JpmcdsDaysDiff(date1, date2, method, &act_days) == FAILURE) goto done;
  ...
  switch (method) {
    ...
    case JPMCDS_ACT_365:
      ...
      if (JpmcdsIsLeap(date1, &isleap) == FAILURE) goto done;
      ...
      if (JpmcdsYear(date1, &start_year) == FAILURE) goto done;

      if (JpmcdsYear(date2, &end_year) == FAILURE) goto done;
      ...
      for (i = start_year + 1; i < end_year; i++) {
        ...
        if (JpmcdsIsLeap(cur_date, &isleap) == FAILURE) goto done;
      ...
      if (start_year != end_year) {
        if (JpmcdsIsLeap(date2, &isleap) == FAILURE) goto done;

        if (JpmcdsYearStart(date2, &cur_date) == FAILURE) goto done;
...
***************************************************************************
*/
int JpmcdsDaysDiff(TDate date1, /* (I) start date */
                   TDate date2, /* (I) end date   */
                   long method, /* (I) method: B30/360
                                   B30E/360
                                   ACT/365
  ...
  switch (method) {
    case JPMCDS_B30_360:
      ...
      if (JpmcdsDateToMDY(date1, &date1MDY) == FAILURE) goto done;

      if (JpmcdsDateToMDY(date2, &date2MDY) == FAILURE) goto done;
...
***************************************************************************
*/
int JpmcdsYearStart(TDate DATE,   /* input date */
                    TDate *result /* output           */
) {
  TMonthDayYear next_year;

  if (JpmcdsDateToMDY(DATE, &next_year) == FAILURE) return FAILURE;

  next_year.month = 1;
  next_year.day = 1;

  if (JpmcdsMDYToDate(&next_year, result) == FAILURE) return FAILURE;
...
***************************************************************************
*/
int JpmcdsYear(TDate DATE, /* input date */
               int *result /* output           */
) {
  TMonthDayYear temp;
...
***************************************************************************
*/
int JpmcdsDaysLeftThisYear(TDate DATE,  /* input date */
  ...
  if (JpmcdsMDYToDate(&next_year, &next_year_days) == FAILURE) return FAILURE;
...
static int mymin(int *values) {
  ...
  for (i = 2; i <= num; i++)
    if (values[i] < minimum_val) minimum_val = values[i];
    ...
    TDate *advAdjustedDate)  /* (O) Advanced adjusted date */
{
  ...
  /* Advance the date */
  if (JpmcdsDtFwdAny(date, interval, &advancedDate) == FAILURE) goto done;

  /* Adjust to find a good business day */
  if (JpmcdsBusinessDay(advancedDate, badDayMethod, holidayFile,
                        advAdjustedDate) == FAILURE)
    goto done;
