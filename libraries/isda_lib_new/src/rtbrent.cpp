...
***************************************************************************
*/
static int brentMethod(TObjectFunc funcd, /* (I) Name of function to call */
                       void *data,        /* (I) Data to pass into funcd */
                       int numIterations, /* (I) Maximum number of iterations*/
...

{
  ...
  if (secantMethod(funcd, data, numIterations, xacc, facc, boundLo, boundHi,
                   xPoints, yPoints, &foundIt, &bracketed,
                   solution) == FAILURE) {
    return FAILURE;
  } else if (foundIt) {
    return SUCCESS;
  ...
  /* Root is still not bracketed, so try at the upper bound now. */
  {
    ...
    {
      JpmcdsErrMsg(
          "%s: Function values (%2.6e,%2.6e) at bounds\n"
...
bracketed:
  ...
  if (brentMethod(funcd, data, numIterations, xacc, facc, xPoints, yPoints,
                  solution) == FAILURE) {
    return FAILURE;
  } else {
    return SUCCESS;
...
***************************************************************************
*/
static int brentMethod(TObjectFunc funcd, /* (I) Function to evaluate */
                       void *data,        /* (I) Data to pass into funcd */
                       int numIterations, /* (I) Maximum number of iterations*/
                       double xacc,       /* (I) X accuracy tolerance */
                       double facc,       /* (I) Function accuracy tolerance */
