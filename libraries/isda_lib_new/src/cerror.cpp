...
***************************************************************************
*/
void JpmcdsErrMsgOn(void) { pWriteMessage = TRUE; }
...
***************************************************************************
*/
void JpmcdsErrMsgOff(void) { pWriteMessage = FALSE; }
...
***************************************************************************
*/
static int FileCreate(char *fileName,  /* (I) */
                      TBoolean append) /* (I) */
{
  TMODE tMode;
  ...
  if (tfp == NULL) {
    if (append) /* Now try again, using JPMCDS_FWRITE */
    {
      ...
      if (tfp == NULL) return FAILURE; /* Now we finally give up */
    } else {
      return FAILURE;
    }
  }
...
enum offsets { /* offsets for starting and end delimiting characters of ctime()
                  output */
               S_MONTH = 4,
               E_MONTH = 7,
               S_YEAR = 20,
...
static int TimeStampFill(time_t aStamp_t) {
  ...
  if ((sb = ctime(&aStamp_t)) != NULL) {
    ...
    if (JpmcdsSprintf(buffer, JPMCDS_ERR_MSG_BUFFER, "-- %s-%s-%s %s --\n",
                      sb + S_DAY, sb + S_MONTH, sb + S_YEAR,
                      sb + S_TIME) == FAILURE) {
      return FAILURE;
    }
    if (JpmcdsWriteToLog(FALSE, buffer, NULL) != SUCCESS) return FAILURE;
  } else {
...
***************************************************************************
*/
static int JpmcdsWriteToLog(TBoolean formatted, char *format,
                            va_list parminfo) {
  char buffer[JPMCDS_ERR_MSG_BUFFER];
  char *bufp = buffer;
...
***************************************************************************
*/
TBoolean JpmcdsErrMsgRecordEnabled(void) { return (record.on); }
...
EXPORT char **JpmcdsErrGetMsgRecord(void) {
  ...
  if (record.on == FALSE) return NULL;

  /*
   *  Determine if there are any error messages yet.
   */
  if (record.lastLine == -1) return record.buf;
