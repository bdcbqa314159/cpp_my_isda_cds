...
*/
static int
JpmcdsObjFunctionRate(                      /* objective func for root finder */
                         ...
                         ZCurve *zc) /* (I) ZCurve to copy */
{
  ...
  TCurve *tc = JpmcdsMakeTCurve(zc->valueDate, /* make TCurve copy of ZCurve */
                                zc->date, zc->rate, zc->numItems,
                                (double)zc->basis, zc->dayCountConv);
  if (tc == NULL)
                           ...
                           TCurve *tc) /* (I) TCurve to copy */
{
  ...
  zc = JpmcdsZCMake(tc->fBaseDate,
                    ...
                    (long)tc->fBasis, tc->fDayCountConv);

  if (zc != NULL) {
    int i; /* loops over elements in TCurve */
    for (i = 0; i < tc->fNumItems; i++) {
      if (JpmcdsZCAddRate(zc, tc->fArray[i].fDate, tc->fArray[i].fRate) ==
          FAILURE) {
        JpmcdsZCFree(zc);
                           ...
                           ZCurve *zc2) /* (I) zCurve to add from */
{
  ...
  for (i = 0; i < zc2->numItems && zc2->date[i] < firstDate; i++) {
    if (JpmcdsZCAddGenRate(zc1, zc2->date[i], zc2->rate[i], zc2->basis,
                           ...
                           long dayCountConv) /* (I) day-count-convention */
{
  ...
  for (i = 0; i < n; i++) {
    if (JpmcdsZCAddGenRate(zc, dates[i], rates[i], JPMCDS_SIMPLE_BASIS,
                           dayCountConv) == FAILURE) {
      JpmcdsErrMsg(
          "JpmcdsZCAddMoneyMarket: error adding rate[%d]: %6.4f%% %s\n", i,
          rates[i], JpmcdsFormatDate(dates[i]));
      return FAILURE;
    }
  }
...
int JpmcdsZCAddCashFlowList(            /* adds cashFlowList info to a ZCurve */
                            ...
                            TCashFlowList
                                *cfl,     /* (I) Cash flows to add to ZCurve */
                            ...
                            long interpType, /* (I) Interpolation method's type
                                              */
                            TInterpData *interpData) /* (I) Interpolation
                                                        method's data */
{
  ...
  if (zc->numItems <= 0) /* if no points in ZCurve ... */
  {
    ...
    if (firstUncovered < 0 || cfl->fArray[firstUncovered].fDate < lastZCDate) {
      JpmcdsErrMsg(
          "%s: no cash flows in list beyond"
  ...
  {
    if (JpmcdsJpmcdsZCPresentValueCFL(zc, cfl, 0, firstUncovered - 1,
      ...
      cfl->fArray[firstUncovered].fDate == date) /* and c.f. is date to add */
  {
    ...
    if (JpmcdsZCAddDiscountFactor(zc, date, discFactor) == FAILURE) goto done;
    ...
    if (zc->numItems <= 0) /* make a guess based on zc */
    {
      ...
      if (JpmcdsZCInterpolate(zc, date, JPMCDS_LINEAR_INTERP, NULL, &rate) ==
          FAILURE)
        rate = 0.06;
      rate = MAX(0.01, MIN(1.00, rate));
    }
    if (JpmcdsZCAddRate(zc, date, rate) == FAILURE) goto done;
    ...
    if (JpmcdsRootFindBrent((TObjectFunc)JpmcdsObjFunctionRate, &objData,
                            MIN_ZERO_RATE, MAX_ZERO_RATE, MAX_ITERATIONS, rate,
                            INITIAL_X_STEP, INITIAL_F_DERIV, X_TOLERANCE,
                            F_TOLERANCE, &rate) == FAILURE) {
      goto done;
    }
  ...
  if (firstUncovered < cfl->fNumItems) {
    if (JpmcdsZCAddCFLPoints(zc, cfl, cfl->fArray[firstUncovered].fDate - 1,
  ...
  if (status == FAILURE)
    JpmcdsErrMsg("%s: Failed for CFL maturing %s.\n", routine,
                 JpmcdsFormatDate(cfl->fArray[cfl->fNumItems - 1].fDate));
...
int JpmcdsZCAddCFLPoints(            /* adds points in a CFL to zcurve */
                         ZCurve *zc, /* (M) ZCurve to add info to */
                         TCashFlowList
                             *cfl,       /* (I) Use dates only for pts to add */
                         TDate lastDate, /* (I) Only add pts after this date*/
                         long interpType, /* (I) Interpolation method's type */
                         TInterpData
                             *interpData) /* (I) Interpolation method's data */
{
  ...
  if (dates == NULL || rates == NULL) goto done;
  ...
  for (i = 0; i < cfl->fNumItems; i++) {
    ...
    {
      double rate;
      if (JpmcdsZCInterpolate(zc, d, interpType, interpData, &rate) ==
          FAILURE) {
        goto done;
      }
      if (rate >= 0.0) {
  ...
  while (--n >= 0) {
    if (JpmcdsZCAddRate(zc, dates[n], rates[n]) == FAILURE) goto done;
  }
...
*/
static int
JpmcdsObjFunctionRate(             /* objective function for root finder */
                      double rate, /* (I) newest rate guess */
                      TObjectiveData
                          *data,      /* (I) representing problem solved */
                      double *result) /* (O) objective function value */
{
  ...
  if (JpmcdsZCComputeDiscount(data->zc, data->zc->date[data->zcIndex], rate,
                              &data->zc->discount[data->zcIndex]) == FAILURE) {
    goto done;
  }

  if (JpmcdsJpmcdsZCPresentValueCFL(data->zc, data->cfl, data->firstUncovered,
                                    data->cfl->fNumItems - 1, data->interpType,
                                    data->interpData, &sumPV) == FAILURE) {
    goto done;
  }
                       ...
                       long dayCountConv) /* (I) day counting convention */
{
  ...
  if (JpmcdsRateToDiscount(rate, /* convert to discount factor */
                           zc->valueDate, date, dayCountConv, basis,
                           &df) == FAILURE)
    goto done;

  if (JpmcdsZCAddDiscountFactor(zc, date, df) == FAILURE) goto done;
                              ...
                              double disc) /* (I) discount factor to add */
{
   ...
   */
  if (JpmcdsDiscountToRate(disc, zc->valueDate, date, zc->dayCountConv,
                           zc->basis, &rate) == FAILURE)
    goto done;
...
** The input rate must be in the proper ZCurve terms (zc->basis and
*zc->dayCountConv).
**
** ZCurve is updated to include the rate.
***************************************************************************
                    ...
                    double rate) /* (I) rate, in ZCurve terms */
{
  ...
  if (JpmcdsZCComputeDiscount(zc, date, rate, &discount) == FAILURE) goto done;
                               ...
                               double disc) /* (I) discount factor to add */
{
  ...
  if (zc->numItems > 0 &&
      date <=
          zc->date[zc->numItems - 1]) { /* make sure date not already in list */
    ...
    if (i >= 0) {
      if (fabs(rate - zc->rate[i]) < 0.0000001) /* if same, no error */
      {
        ...
        JpmcdsErrMsg(
            "%s: Date %s (rate:%6.4f%%) already in curve"
            " at index %d\n (rate:%6.4f%%)\n",
            routine, JpmcdsFormatDate(date), rate * 100.0, i, zc->rate[i]);
        return FAILURE;
      }
    }
  }

  if (zc->numAlloc <= zc->numItems) { /* re-allocate arrays */
    ...
    memcpy((char *)dateNew, (char *)(zc->date), sizeof(TDate) * (zc->numItems));
    memcpy((char *)rateNew, (char *)(zc->rate), sizeof(double) * zc->numItems);
    memcpy((char *)discNew, (char *)(zc->discount),
...
int JpmcdsZCMoneyMarketRate(            /* gets money market rate from ZCurve */
                            ...
                            TInterpData *interpData, /* (I) interpolation method
                                                        to use */
                            double *rate)            /* (O) money market rate */
{
  return JpmcdsZCForwardRate(zc, zc->valueDate, date, JPMCDS_SIMPLE_BASIS,
                             dayCountConv, interpType, interpData, rate);
}
...
int JpmcdsZCForwardRate(                   /* gets forward rate from ZCurve */
                        ...
                        long interpType,   /* (I) interpolation type to use */
                        TInterpData
                            *interpData, /* (I) interpolation method to use*/
                        double *rate)    /* (O) forward rate */
{
  double discount; /* discount factor over forward */

  if (JpmcdsZCFwdDiscountFactor(zc, date1, date2, interpType, interpData,
                                &discount) == FAILURE) {
  ...
  if (discount <= 0.0) /* bad discount factor */
  {
    ...
    if (JpmcdsDiscountToRate(discount, date1, date2, dayCountConv, basis,
                             rate) == FAILURE) {
...
int JpmcdsZCFwdDiscountFactor(/* discount factor for given forward */
                              ...
                              TInterpData *interpData, /* (I) interpolation
                                                          method to use */
                              double
                                  *discount) /* (O) forward discount factor */
{
  double discount1; /* discount factor valueDate to date1 */
  double discount2; /* discount factor valueDate to date2 */

  if (JpmcdsZCDiscountFactor(zc, date1, interpType, interpData, &discount1) ==
      FAILURE) {
    JpmcdsErrMsg("JpmcdsZCFwdDiscountFactor: couldn't calc discount1\n");
    return FAILURE;
  }

  if (JpmcdsZCDiscountFactor(zc, date2, interpType, interpData, &discount2) ==
      FAILURE) {
                                                    ...
                                                    flows*/
{
  ...
  for (i = iLo; i <= iHi; i++) {
    ...
    if (j < zc->numItems && zc->date[j] == date) /* found exact date in zc */
    {
      ...
      if (JpmcdsZCPresentValue(zc, amt, date, interpType, interpData, &pv) ==
          FAILURE)
        goto done;
    }
    sumPV += pv;
...
int JpmcdsZCPresentValue(                 /* present-value of future payment */
                         ...
                         long interpType, /* (I) type of interpolation */
                         TInterpData
                             *interpData, /* (I) data for interpolation */
                         double *pv)      /* (I) present value of price */
{
  ...
  if (JpmcdsZCDiscountFactor(zc, date, interpType, interpData, &discFactor) ==
      FAILURE)
    goto done;
...
int JpmcdsZCInterpolate(                   /* interpolation on a ZCurve */
                        ...
                        TInterpData
                            *interpDataIn, /* (I) data for interp method */
                        double *rate)      /* (O) rate to return */
{
   ...
   */
  if (JpmcdsBSearchLongFast((double)date, zc->date, sizeof(TDate), zc->numItems,
                            &lo, &hi) == FAILURE) {
    goto done;
  }
    ...
    case JPMCDS_FLAT_FORWARDS: {
      ...
      if (hi_lo == 0) {
        *rate = zc->rate[lo];
        ...
        if (JpmcdsZCComputeDiscount(zc, zc->date[lo], zc->rate[lo], &discLo) ==
            FAILURE)
          goto done;

        if (JpmcdsZCComputeDiscount(zc, zc->date[hi], zc->rate[hi], &discHi) ==
            FAILURE)
          goto done;
        ...
        if (JpmcdsDiscountToRate(discDate, zc->valueDate, date,
                                 zc->dayCountConv, zc->basis, rate) == FAILURE)
          goto done;
      } /* else */
    } break;
...
** speed (since this routine is critical to performance of building
*zero-curves).
...
int JpmcdsZCComputeDiscount(             /* calculate discount from ZC rate */
                            ...
                            double *discount) /* (O) discount factor */
{
  if (zc->basis == 1 && rate >= -1.0 && date >= zc->valueDate &&
      (zc->dayCountConv == JPMCDS_ACT_365F ||
       zc->dayCountConv == JPMCDS_ACT_360)) {
    *discount =
        pow(1 + rate, (zc->valueDate - date) /
                          (zc->dayCountConv == JPMCDS_ACT_360 ? 360.0 : 365.0));
    return SUCCESS;
  }

  if (JpmcdsRateToDiscount(rate, zc->valueDate, date, zc->dayCountConv,
                           zc->basis, discount) == FAILURE) {
