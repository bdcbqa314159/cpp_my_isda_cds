    ...
    char *holidayFile)  /* (I) Holiday file                     */
{
  ...
  if (cashDates == NULL || swapDates == NULL || cashRates == NULL ||
      swapRates == NULL)
    goto done;
  ...
  for (i = 0; i < nInstr; i++) {
    ...
    if (instr != JpmcdsMONEYNAME && instr != JpmcdsSWAPNAME) {
      JpmcdsErrMsg(
          "%s: unknown instrument type (%c)."
          " Only (M)oney market or (S)wap is allowed.\n",
          routine, instrNames[i]);
      goto done;
    }

    if (instr == JpmcdsMONEYNAME) { /* MM Rate */
  ...
  if (zcurveIni == NULL) goto done;

  /* Cash instruments */
  zcurveCash = JpmcdsZCCash(zcurveIni, cashDates, cashRates, nCash, mmDCC);
  if (zcurveCash == NULL) goto done;

  /* Swap instruments */
  zcurveSwap = JpmcdsZCSwaps(zcurveCash, NULL, /* discZC */
                             swapDates, swapRates, nSwap, fixedSwapFreq,
                             floatSwapFreq, fixedSwapDCC, floatSwapDCC,
                             fwdLength, badDayConv, holidayFile);
  if (zcurveSwap == NULL) goto done;
