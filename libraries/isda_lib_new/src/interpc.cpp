...
*/
static int
InterpRateDiscountFactors(/* Used for basis=JPMCDS_DISCOUNT_FACTOR */
                          double desiredDate, /* (I) Date to interpolate at */
                          TCurve *curve,      /* (I) Curve with rates */
                          double *rate);      /* (O) Interpolated rate */
...
***************************************************************************
*/
static int InterpRateFind(double date, /* (I) Date to interpolate at */
                     ...
                     double *rate)      /* (O) Interpolated rate */
{
  ...
  if (JpmcdsInterpRateExact((double)desiredDate, curve, interpType, rate) ==
      FAILURE)
    goto done;
  ...
  */
  ...
  switch (interpType) {
    case JPMCDS_FLAT_FORWARDS:
      if (InterpRateFlatFwds(desiredDate, curve, rate) == FAILURE) goto done;
      break;
    ...
    case JPMCDS_LINEAR_INTERP:
      ...
      if (JpmcdsLinInterpLongPoint1(&curve->fArray[0].fDate, sizeof(TRatePt),
                                    curve->fNumItems, &curve->fArray[0].fRate,
                                    sizeof(TRatePt), desiredDate,
                                    (TMetricDoubleFunc)NULL, rate) == FAILURE)
        goto done;
      break;
    ...
    double *rate)       /* (O) Interpolated rate */
{
  ...
  if (InterpRateFind(desiredDate, curve, &lo, &hi) == FAILURE) goto done;
  ...
  if (JpmcdsRateToDiscount(rateLo, curve->fBaseDate, dateLo,
                           curve->fDayCountConv, (long)curve->fBasis,
                           &discLo) == FAILURE)
    goto done;

  if (JpmcdsRateToDiscount(rateHi, curve->fBaseDate, dateHi,
                           curve->fDayCountConv, (long)curve->fBasis,
                           &discHi) == FAILURE)
    goto done;
  ...
  switch (curve->fDayCountConv) {
    ...
    case JPMCDS_ACT_365:
      ...
      if (isFractionalDate) {
        JpmcdsErrMsg(
            "%s: Unsupported curve daycount convention %s for "
            "fractional date %f\n",
            routine, JpmcdsFormatDayCountConv(curve->fDayCountConv),
            desiredDate);
        goto done;
      }
      if (JpmcdsDaysDiff(dateLo, desiredTDate, curve->fDayCountConv, &diffh) !=
          SUCCESS)
        goto done;

      if (JpmcdsDaysDiff(dateLo, dateHi, curve->fDayCountConv, &diffl) !=
          SUCCESS)
        goto done;
  ...
  */
  if (isFractionalDate) {
    ...
    if (JpmcdsDiscountToRateYearFrac(discDate, desiredDateYearFrac,
                                     curve->fBasis, rate) != SUCCESS) {
      goto done;
    }
  } else {
    /* Not a fractional date, so use DiscountToRate */
    if (JpmcdsDiscountToRate(discDate, curve->fBaseDate, desiredTDate,
                             curve->fDayCountConv, (long)curve->fBasis,
                             rate) == FAILURE)
      goto done;
  }
    ...
    double *rate)       /* (O) Interpolated rate */
{
  ...
  if (JpmcdsDiscountToRate(discLo, curve->fBaseDate, dateLo, JPMCDS_ACT_365F,
                           JPMCDS_ANNUAL_BASIS, &rateLo) == FAILURE)
    goto done;

  if (JpmcdsDiscountToRate(discHi, curve->fBaseDate, dateHi, JPMCDS_ACT_365F,
                           JPMCDS_ANNUAL_BASIS, &rateHi) == FAILURE)
    goto done;
  ...
  {
    rateDate =
        ((rateHi - rateLo) / (dateHi - dateLo)) * (desiredDate - dateLo) +
        rateLo;
  }
  /*
  ...
  */
  if (isFractionalDate) {
    ...
    if (JpmcdsRateToDiscountYearFrac(rateDate, desiredDateYearFrac,
                                     JPMCDS_ANNUAL_BASIS, rate) != SUCCESS) {
      goto done;
    }
  } else {
    /* Not a fractional date, so use RateToDiscount */
    if (JpmcdsRateToDiscount(rateDate, curve->fBaseDate, desiredTDate,
                             JPMCDS_ACT_365F, JPMCDS_ANNUAL_BASIS,
                             rate) == FAILURE)
      goto done;
  }
                          ...
                          int *hi)     /* (O) upper bracketing point */
{
  ...
  if (JpmcdsBSearchLong(date, &tc->fArray[0].fDate, sizeof(TRatePt),
                        tc->fNumItems, lo, hi) == FAILURE)
    goto done;
  status = SUCCESS;
