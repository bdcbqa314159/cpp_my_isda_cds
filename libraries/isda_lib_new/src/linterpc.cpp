...
TCurve *JpmcdsLinInterpCurve(
  ...
  if (interpCurve == (TCurve *)NULL) goto done;
  ...
  for (idx = 0; idx < desiredDates->fNumItems; idx++) {
    ...
    modDates[idx] = MAX(modDates[idx], oldCurve->fArray[0].fDate);
  ...
  if (JpmcdsLinInterpLongArray1(
          ...
          &oldCurve->fArray[0].fRate, sizeof(TRatePt), modDates, sizeof(TDate),
          desiredDates->fNumItems,
          JpmcdsDayCountToMetricFunc(oldCurve->fDayCountConv), sizeof(TRatePt),
          &interpCurve->fArray[0].fRate) == FAILURE) {
    JpmcdsFreeTCurve(interpCurve);
    interpCurve = NULL;
    goto done;
  }
  ...
  FREE_ARRAY(modDates);
  if (interpCurve == NULL) JpmcdsErrMsg("%s: Failed.\n", routine);
