...
static int cdsBootstrapPointFunction(double cleanSpread, void *data,
                                     double *pv);
...
EXPORT TCurve *JpmcdsCleanSpreadCurve(
  ...
  if (couponInterval == NULL) couponInterval = &ivl3M;
  ...
  out =
      CdsBootstrap(today, discountCurve, startDate, stepinDate, cashSettleDate,
                   nbDate, endDates, couponRates, recoveryRate, payAccOnDefault,
                   couponInterval, paymentDCC, stubType, badDayConv, calendar);

done:
  FREE(includeEndDates);
  FREE(includeCouponRates);
  if (out == NULL) JpmcdsErrMsgFailure(routine);
...
static TCurve *CdsBootstrap(
  ...
  cdsCurve = JpmcdsMakeTCurve(today, endDates, couponRates, nbDate,
                              JPMCDS_CONTINUOUS_BASIS, JPMCDS_ACT_365F);
  ...
  for (i = 0; i < nbDate; ++i) {
    ...
    cl = JpmcdsCdsContingentLegMake(MAX(today, startDate), endDates[i], 1.0,
                                    protectStart);
    if (cl == NULL) goto done;

    fl = JpmcdsCdsFeeLegMake(startDate, endDates[i], payAccOnDefault,
                             couponInterval, stubType, 1.0, couponRates[i],
                             paymentDCC, badDayConv, calendar, protectStart);
    ...
    if (JpmcdsRootFindBrent((TObjectFunc)cdsBootstrapPointFunction,
                            (void *)&context, 0.0, /* boundLo */
                            1e10,                  /* boundHi */
                            100,                   /* numIterations */
                            guess, 0.0005,         /* initialXstep */
                            0,                     /* initialFDeriv */
                            1e-10,                 /* xacc */
                            1e-10,                 /* facc */
                            &spread) != SUCCESS) {
      JpmcdsErrMsg("%s: Could not add CDS maturity %s spread %.2fbp\n", routine,
                   JpmcdsFormatDate(endDates[i]), 1e4 * couponRates[i]);
      goto done;
    }
    ...
    if (i > 0) {
      double fwdPrice =
          JpmcdsForwardZeroPrice(cdsCurve, endDates[i - 1], endDates[i]);

      if (fwdPrice > 1) {
        JpmcdsErrMsg(
            "%s: Negative forward hazard rate at maturity %s with "
            "spread %.2fbp\n",
            routine, JpmcdsFormatDate(endDates[i]), 1e4 * couponRates[i]);
        goto done;
      }
    }
  }
...
static int cdsBootstrapPointFunction(double cleanSpread, void *data,
  ...
  if (JpmcdsContingentLegPV(cl, cdsBaseDate, cashSettleDate, stepinDate,
                            discountCurve, cdsCurve, recoveryRate,
                            &pvC) != SUCCESS)
    goto done;

  if (JpmcdsFeeLegPV(fl, cdsBaseDate, stepinDate, cashSettleDate, discountCurve,
                     cdsCurve, isPriceClean, &pvF) != SUCCESS)
    goto done;
...
int CreditCurveConvertRateType(TCurve *cc, long newRateType) {
  ...
  if (IS_EQUAL(newRateType, cc->fBasis)) {
    ...
    for (i = 0; i < cc->fNumItems; ++i) {
      if (JpmcdsConvertCompoundRate(
              cc->fArray[i].fRate, cc->fBasis, cc->fDayCountConv, newRateType,
              cc->fDayCountConv, &cc->fArray[i].fRate) != SUCCESS)
        goto done;
    }
