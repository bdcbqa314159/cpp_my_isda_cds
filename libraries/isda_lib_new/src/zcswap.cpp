...
***************************************************************************
*/
static int CalcPV(TCurve *zeroCurve, /* (I) Zero curve for estim cfls  */
...
int JpmcdsZCAddSwaps(                   /* adds a strip of swaps to ZCurve */
                     ...
                     TBadDayList
                         *badDayList, /* (I) Bad-good day pairs; if non-NULL */
                     /*     maturity dates must be adjusted */
                     TBadDayAndStubPos
                         badDayAndStubPos, /* (I) See JpmcdsBusinessDay      */
                     char *holidayFile)    /* (I) See JpmcdsBusinessDay      */
{
  ...
  if (JpmcdsBadDayAndStubPosSplit(badDayAndStubPos, &badDayConv, &stubPos) !=
      SUCCESS)
    goto done;

  if (badDayList != NULL && badDayConv != JPMCDS_BAD_DAY_NONE) {
    JpmcdsErrMsg(
        "%s: Bad days can be defined either by badDayList or\n"
  ...
  {
    swapDates = JpmcdsSwapDatesNewFromAdjusted(zc->valueDate, fixedSwapFreq,
                                               inDates, numSwaps, badDayList);
    if (swapDates == NULL) goto done;
  } else /* Dates NOT adjusted */
  {
    swapDates = JpmcdsSwapDatesNewFromOriginal(zc->valueDate, fixedSwapFreq,
                                               inDates, numSwaps, badDayList,
  ...
  for (i = 0; i < swapDates->numDates; i++) {
     ...
     */
    if (zc->numItems > 0 &&
        swapDates->adjusted[i] > zc->date[zc->numItems - 1]) {
      /* Check if optimization okay. Note linear forwards
       * not OK because they must include intermed. fwds.
       */
      if (oneAlreadyAdded && discZC == NULL && swapRates[i - 1] != 0.0 &&
          swapDates->adjusted[i - 1] == zc->date[zc->numItems - 1] &&
          swapDates->previous[i] == swapDates->original[i - 1] &&
          swapDates->onCycle[i] && interpType != JPMCDS_LINEAR_FORWARDS) {
        /* Optimization: compute from last
         */
        if (AddSwapFromPrevious(zc, swapDates->adjusted[i], swapRates[i], 1.0,
                                swapDates->adjusted[i - 1], swapRates[i - 1],
                                1.0, fixDayCountConv) == FAILURE) {
          goto done;
        }
      } else /* No efficiency */
      {
        if (JpmcdsZCAddSwap(zc, discZC, 1.0, swapDates->original[i],
                            swapDates->onCycle[i], swapRates[i], fixedSwapFreq,
                            floatSwapFreq, fixDayCountConv, floatDayCountConv,
                            interpType, interpData, badDayList,
                            badDayAndStubPos, holidayFile) == FAILURE) {
          goto done;
        }
        oneAlreadyAdded = TRUE;
      }
    } /* if (zc->numItems > 0 && swapDates->adjusted[i] > zc->date[zc->numItems
         - 1]) */
    else {
      if (JpmcdsZCAddSwap(zc, discZC, 1.0, swapDates->original[i],
                          swapDates->onCycle[i], swapRates[i], fixedSwapFreq,
                          floatSwapFreq, fixDayCountConv, floatDayCountConv,
                          interpType, interpData, badDayList, badDayAndStubPos,
                          holidayFile) == FAILURE) {
        goto done;
      }
      oneAlreadyAdded = TRUE;
...
**        price = discount[n] + couponRate * (sum of
*discount[i]*yearFraction[i])
**
...
**        priceNew = discount[n] * (1 + couponRate*yearFraction[n]) +
*cRate*sumDY
                               ...
                               long dayCount) /* (I) day counting convention */
{
  ...
  if (discount <= 0.0) {
    ...
    JpmcdsErrMsg("%s: Implied discount factor (%f) on %s is <= 0.0.\n", routine,
                 discount, JpmcdsFormatDate(dateNew));
    JpmcdsErrMsg(
        "%s: The swap rates may be inconsistent with "
        "one another.\n",
        routine);
    goto done;
  }
  ...
  if (JpmcdsZCAddDiscountFactor(zc, dateNew, discount) == FAILURE) goto done;
  ...
  if (status == FAILURE) {
    JpmcdsErrMsg("%s: Failed for swap at %s(adj), rate=%f price=%f.\n", routine,
                 JpmcdsFormatDate(dateNew), rateNew, priceNew);
  }
  return status;
}
...
int JpmcdsZCAddSwap(                         /* adds a swap to ZCurve */
                    ...
                    TBadDayAndStubPos
                        badDayAndStubPos, /* (I) Bad day conv + stub pos   */
                    char *holidayFile)    /* (I) See JpmcdsBusinessDay        */
{
  ...
  if (discZC == NULL) {
     ...
     */
    if (JpmcdsFreq2TDateInterval(fixedSwapFreq, &ivl) != SUCCESS) goto done;

    if (onCycle) {
      isEndStub = TRUE;
    } else {
      if (JpmcdsIsEndStub(zc->valueDate, matDate, &ivl, stubPos, &isEndStub) !=
          SUCCESS) {
        goto done;
      }
    }
    cfl = JpmcdsZCGetSwapCFL(zc->valueDate, matDate, isEndStub, rate, &ivl,
                             fixDayCountConv, badDayList, badDayConv,
                             holidayFile);
     ...
     */
    if (JpmcdsZCAddCashFlowList(zc, cfl, price, adjMatDate, interpType,
                                interpData) == FAILURE)
      goto done;
     ...
     */
    if (JpmcdsZCAddCFLPoints(zc, cfl, 0,
                             interpType == JPMCDS_LINEAR_FORWARDS
                                 ? JPMCDS_LINEAR_INTERP
                                 : interpType,
                             interpData) == FAILURE)
      goto done;
  } else /* Need to value floating side */
  {
    if (JpmcdsZCValueFixFltSwap(zc, discZC, price, matDate, rate, fixedSwapFreq,
                                floatSwapFreq, fixDayCountConv,
                                floatDayCountConv, interpType, interpData,
                                badDayAndStubPos, holidayFile) == FAILURE) {
      goto done;
    }
  } /* ValueFloating */
...

{
  ...
  if (JpmcdsFreq2TDateInterval((long)fixedSwapFreq, &fixedInterval) ==
      FAILURE) {
    PROGRAM_BUG();
    goto done;
  }
  if (JpmcdsFreq2TDateInterval((long)floatSwapFreq, &floatInterval) ==
      FAILURE) {
    PROGRAM_BUG();
    goto done;
  }
  ...
  if (JpmcdsIsEndStub(zc->valueDate, matDate, &fixedInterval, stubPos,
                      &isEndStub) != SUCCESS) {
    goto done;
  }
  fixedDL = JpmcdsNewCouponDatesSwap(
      zc->valueDate, matDate, &fixedInterval, ADJUST_LAST_ACC_DATE, IN_ARREARS,
      PAY_OFFSET, RESET_OFFSET, isEndStub, LONG_STUB, FIRST_ROLL_DATE,
      LAST_ROLL_DATE, FULL_FIRST_COUPON_DATE, badDayConv, badDayConv,
      badDayConv, holidayFile);
  if (fixedDL == NULL) goto done;

  /* Make notional of fixed side = +1.
   */
  fixedStream = JpmcdsNewStreamFixed(fixedDL, JPMCDS_SINGLE_REFIX, 1.0,
                                     fixDayCountConv, 0, rate, rate, rate);
  if (fixedStream == NULL) goto done;

  fixedCFL = JpmcdsNewStreamFixedCFL(zc->valueDate, fixedStream);
  if (fixedCFL == NULL) goto done;

  if (JpmcdsIsEndStub(zc->valueDate, matDate, &floatInterval, stubPos,
                      &isEndStub) != SUCCESS) {
    goto done;
  }
  floatDL = JpmcdsNewCouponDatesSwap(
      zc->valueDate, matDate, &floatInterval, ADJUST_LAST_ACC_DATE, IN_ARREARS,
      PAY_OFFSET, RESET_OFFSET, isEndStub, LONG_STUB, FIRST_ROLL_DATE,
      LAST_ROLL_DATE, FULL_FIRST_COUPON_DATE, badDayConv, badDayConv,
      badDayConv, holidayFile);
  if (floatDL == NULL) goto done;

  /* Make notional of floating side -1.
   */
  floatStream = JpmcdsNewStreamFloat(floatDL, JPMCDS_SINGLE_REFIX, -1.0,
                                     floatDayCountConv, 0, &firstCoupon,
                                     &finalCoupon, 0.0, /* Average so far */
                                     1.0,               /* Compounded so far */
                                     &floatingRate);

  if (floatStream == NULL) goto done;

  /* find the furthest reaching date */

  if (JpmcdsDtFwdAny(floatDL->fArray[floatDL->fNumItems - 1].resetDate,
                     &floatInterval, &indexMatDate) == FAILURE)
    goto done;

  if (JpmcdsBusinessDay(indexMatDate, badDayConv, holidayFile, &indexMatDate) ==
      FAILURE)
    goto done;
  ...
  if (JpmcdsZCAddRate(zc, maxDate, rate) == FAILURE) goto done;
   ...
   */
  if (JpmcdsRootFindBrent(JpmcdsObjFuncPVtheSwap, &funcParams, LOWER_BOUND,
                          UPPER_BOUND, MAX_ITERATIONS, rate, INITIAL_X_STEP,
                          INITIAL_F_DERIV, X_TOLERANCE, F_TOLERANCE,
                          &zc->rate[zc->numItems - 1]) == FAILURE) {
    JpmcdsErrMsg("%s: Root finder failed.\n", routine);
    goto done;
  }

  if (JpmcdsZCComputeDiscount(zc, zc->date[zc->numItems - 1],
   ...
   */
  floatCFL = JpmcdsNewStreamFloatCFLGen(tc, interpType, floatStream, badDayConv,
                                        holidayFile);
  if (floatCFL == NULL) goto done;

  swapCFL = JpmcdsMergeCFL(fixedCFL, floatCFL);
  if (swapCFL == NULL) goto done;
   ...
   * created by this routine, the swap will be priced to par.
   */
  if (JpmcdsZCAddCFLPoints(zc, swapCFL, 0, /* Add point after this one*/
  ...
  if (minDate != maxDate) {
    ...
    if (JpmcdsZCInterpolate(zc, minDate, interpType, interpData, &zeroRate) ==
        FAILURE)
      goto done;

    if (JpmcdsZCAddRate(zc, minDate, zeroRate) == FAILURE) goto done;
  }
    ...
    double *f)        /* (O) f(x) == value to make zero   */
{
  ...
  if (CalcPV(params->zeroCurve, params->interpType, params->fixedCfls,
             params->floatStream, params->price, params->rateBadDayConv,
             params->holidayFile, params->discZC, f) == FAILURE) {
    *f = -1.0;
    return FAILURE;
  } else
    return SUCCESS;
                  ...
                  double *dealPV)      /* (O) PV of the swap             */
{
  ...
  floatFlows = JpmcdsNewStreamFloatCFLGen(zeroCurve, interpType, floatStream,
                                          rateBadDayConv, holidayFile);
  if (floatFlows == NULL) goto done;
