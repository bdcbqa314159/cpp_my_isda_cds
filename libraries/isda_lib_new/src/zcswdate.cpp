    ...
    TBadDayList *badDayList) /* (I) Maps good to bad & vice-vsa*/
{
  ...
  if (sd == NULL) goto done;

  /* For each swap maturity date... */
  for (idx = 0; idx < numDates; idx++) {
    sd->adjusted[idx] = adjustedDates[idx];
    sd->original[idx] =
        JpmcdsGood2BadBadDayList(badDayList, adjustedDates[idx]);

    /* Now set sd->onCycle[idx] and sd->previous[idx]. */
    if (SetPrevDateAndOnCycle(valueDate, sd->original[idx], freq,
    ...
    char *holidayFile)       /* (I) List of holidays */
{
  ...
  for (idx = 0; idx < numDates; idx++) {
    ...
    if (JpmcdsZCAdjustDate(originalDates[idx], badDayList, badDayConv,
                           holidayFile, &sd->adjusted[idx]) == FAILURE)
      goto done;

    /* Now set sd->onCycle[idx] and sd->previous[idx]. */
    if (SetPrevDateAndOnCycle(valueDate, originalDates[idx], freq,
                              &sd->onCycle[idx], &sd->previous[idx]) == FAILURE)
      goto done;
  }
...
TSwapDates *JpmcdsSwapDatesNewEmpty(int numDates) {
  ...
  if (numDates > 0) {
    ...
    if (sd->adjusted == NULL || sd->original == NULL || sd->previous == NULL ||
        sd->onCycle == NULL)
      goto done;
  } else {
    sd->adjusted = NULL;
    ...
    TDate *prevDate)   /* (O) Previous coupon date */
{
  ...
  if (JpmcdsFreq2TDateInterval(freq, &interval) == FAILURE) goto done;

  if (JpmcdsDateToMDY(valueDate, &valueMDY) == FAILURE) goto done;

  if (JpmcdsDateToMDY(origDate, &origMDY) == FAILURE) goto done;

  if ((valueMDY.day <= 28) && (origMDY.day <= 28)) {
    /*
     ...
     */
    if (JpmcdsCountDates(valueDate, origDate, &interval, &numIntervals,
                         &extraDays) == FAILURE)
      goto done;
  ...
  if (*onCycle) /* On cycle */
  {
    if (JpmcdsDateFromDateAndOffset(valueDate, &interval, numIntervals - 1,
                                    prevDate) == FAILURE)
      goto done;
  } else /* Off cycle */
  {
    if (JpmcdsDateFromDateAndOffset(origDate, &interval, -1, prevDate) ==
        FAILURE)
      goto done;
  }
  status = SUCCESS;
...
***************************************************************************
*/
int JpmcdsZCAdjustDate(TDate date,              /* (I) Date to be adjusted */
