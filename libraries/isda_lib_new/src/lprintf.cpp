...
int JpmcdsVfprintf(TFile *tFile, char *fmt, va_list arg) {
  ...
  conv:
    switch (c) {
      ...
      default:
        if (c != '%') goto done;
      copy1:
        if (JpmcdsLocalPutc((char)c, tFile) < 0) return EOF;
        ++nwritten;
        continue;
    }
    ...
    if (i < F.fieldWidth && !F.leftJustify) {
      do {
        if (JpmcdsLocalPutc(' ', tFile) < 0) return EOF;
        ++nwritten;
    ...
    if (i != 0) {
      if (JpmcdsLocalFwrite(s, i, tFile) != i) return EOF;
    }
    nwritten += i;
  ...
  if (tFile->type == TFILE_STRING) JpmcdsLocalPutc('\0', tFile);
