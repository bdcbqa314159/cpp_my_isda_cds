...
***************************************************************************
*/
static int CheckArgsSwapMakeSchedule(TDate startDate, TDate matDate,
                                     long payBadDayConv, long accBadDayConv,
                                     long resetBadDayConv, int resetOffsetDays,
                                     TDate firstRollDate, TDate lastRollDate,
    ...
    char *holidayFile)             /* (I) See JpmcdsBusinessDay           */
{
  ...
  if (CheckArgsSwapMakeSchedule(startDate, matDate, payBadDayConv,
                                accBadDayConv, resetBadDayConv, resetOffsetDays,
                                firstRollDate, lastRollDate,
  ...
  if (JpmcdsGenerateUnadjDates(startDate, matDate, couponInterval, stubAtEnd,
                               longStub, firstRollDate, lastRollDate,
                               &unadjDates, &stubInfo) == FAILURE) {
  ...
  for (i = 0; i < unadjDates->fNumItems; i++) {
    if (!(i == unadjDates->fNumItems - 1) || adjustLastAccDate == TRUE) {
      if (JpmcdsBusinessDay(unadjDates->fArray[i], accBadDayConv, holidayFile,
                            &tempAccDate) == FAILURE) {
  ...
  for (i = 0, j = 0; i < adjStreamSched->fNumItems; i++) {
    if (adjStreamSched->fArray[i].accrueStartDate !=
      ...
      if (i == 0) {
        ...
        } else if (adjStreamSched->stubLocation == JPMCDS_TWO_STUB) {
      ...
      } else if (i == adjStreamSched->fNumItems - 1) {
  ...
  if (j != streamSched->fNumItems) {
    JpmcdsErrMsg(
        "%s: Error in number of dates in stream: \n"
        "Index=> %d, fNumItems=%d\n",
  ...
  for (i = 0; i < streamSched->fNumItems; i++) {
    if (JpmcdsBusinessDay(streamSched->fArray[i].accrueEndDate, payBadDayConv,
                          holidayFile,
  ...
  if (payOffsetDays != 0) {
    for (i = 0; i < streamSched->fNumItems; i++) {
      if (JpmcdsDateFromBusDaysOffset(
              streamSched->fArray[i].payDate, payOffsetDays, holidayFile,
              &streamSched->fArray[i].payDate) == FAILURE) {
  ...
  for (i = 0; i < streamSched->fNumItems; i++) {
    ...
    if (JpmcdsBusinessDay(streamSched->fArray[i].resetDate, resetBadDayConv,
                          holidayFile,
    ...
    if (resetOffsetDays != 0) {
      if (JpmcdsDateFromBusDaysOffset(
              streamSched->fArray[i].resetDate, -resetOffsetDays, holidayFile,
              &streamSched->fArray[i].resetDate) == FAILURE) {
        JpmcdsErrMsg("%s: error when calculating reset date.\n", routine);
        goto error;
      }
     ...
     */
    while (streamSched->fArray[i].resetDate > streamSched->fArray[i].payDate) {
      if (JpmcdsBusinessDay(--streamSched->fArray[i].resetDate,
                            JPMCDS_BAD_DAY_PREVIOUS, holidayFile,
                            &streamSched->fArray[i].resetDate) == FAILURE) {
  ...
  if (streamSched->stubLocation > JPMCDS_BACK_STUB && fullFirstCoupDate > 0) {
...
static int CheckArgsSwapMakeSchedule(TDate startDate, TDate matDate,
  ...
  if (startDate > matDate) {
    JpmcdsErrMsg("%s: maturity date (%s) is on or before start date (%s)\n",
                 routine, JpmcdsFormatDate(matDate),
  ...
  if (fullFirstCoupDate > startDate) {
    JpmcdsErrMsg(
        "%s: full first coupon date (%s) is after"
        "swap start date (%s).\n",
        routine, JpmcdsFormatDate(fullFirstCoupDate),
        JpmcdsFormatDate(startDate));
    return FAILURE;
  }
  ...
  if (firstRollDate > 0) {
    if (lastRollDate > 0) {
      JpmcdsErrMsg(
          "%s: firstRollDate(%s) and lastRollDate(%s) cannot"
          "both be defined.\n",
          routine, JpmcdsFormatDate(firstRollDate),
    ...
    if (firstRollDate > matDate) {
      JpmcdsErrMsg("%s: firstRollDate(%s) is after maturity (%s).\n", routine,
                   JpmcdsFormatDate(firstRollDate), JpmcdsFormatDate(matDate));
      return FAILURE;
    }
  }
  if (lastRollDate > 0) {
    if (lastRollDate < startDate) {
      JpmcdsErrMsg("%s: lastRollDate(%s) is before start date (%s).\n", routine,
                   JpmcdsFormatDate(lastRollDate), JpmcdsFormatDate(startDate));
      return FAILURE;
    }
    if (lastRollDate > matDate) {
      JpmcdsErrMsg("%s: lastRollDate(%s) is after maturity (%s).\n", routine,
                   JpmcdsFormatDate(lastRollDate), JpmcdsFormatDate(matDate));
      return FAILURE;
    }
  }
...
int JpmcdsCouponDateListCheck(TCouponDateList *schedule) {
  ...
  if (schedule->fNumItems < 1) {
    JpmcdsErrMsg(
        "%s: schedule must have at least one coupon period"
        " but has only %d.\n",
        routine, schedule->fNumItems);
    return FAILURE;
  }
  ...
  if (schedule->fArray[schedule->fNumItems - 1].accrueEndDate <
    ...
    JpmcdsErrMsg("%s: swap start date (%s) is after swap end date (%s).\n",
                 routine, JpmcdsFormatDate(schedule->fArray[0].accrueStartDate),
                 JpmcdsFormatDate(
                     schedule->fArray[schedule->fNumItems - 1].accrueEndDate));
    return FAILURE;
  }
  ...
  for (i = 0; i < schedule->fNumItems; i++) {
    if (schedule->fArray[i].accrueEndDate <
      ...
      JpmcdsErrMsg(
          "%s: coupon #%d has accrue end date (%s) before"
          " accrue start date (%s).\n",
          routine, i + 1, JpmcdsFormatDate(schedule->fArray[i].accrueEndDate),
  ...
  for (i = 1; i < schedule->fNumItems; i++) {
    if (schedule->fArray[i - 1].accrueStartDate >
      ...
      JpmcdsErrMsg(
          ...
          i, JpmcdsFormatDate(schedule->fArray[i - 1].accrueStartDate), i + 1,
          JpmcdsFormatDate(schedule->fArray[i].accrueStartDate));
      return FAILURE;
    }
    if (schedule->fArray[i - 1].accrueEndDate >
      ...
      JpmcdsErrMsg("%s: acc end date #%d (%s) > acc end date #%d (%s).\n",
                   ...
                   i + 1, JpmcdsFormatDate(schedule->fArray[i].accrueEndDate));
      return FAILURE;
    }
    if (schedule->fArray[i - 1].payDate > schedule->fArray[i].payDate) {
      JpmcdsErrMsg("%s: payment date #%d (%s) > payment date #%d (%s).\n",
                   routine, i,
                   JpmcdsFormatDate(schedule->fArray[i - 1].payDate), i + 1,
                   JpmcdsFormatDate(schedule->fArray[i].payDate));
      return FAILURE;
    }
    if (schedule->fArray[i - 1].resetDate > schedule->fArray[i].resetDate) {
      JpmcdsErrMsg("%s: reset date #%d (%s) > reset date #%d (%s).\n", routine,
                   i, JpmcdsFormatDate(schedule->fArray[i - 1].resetDate),
                   i + 1, JpmcdsFormatDate(schedule->fArray[i].resetDate));
      return FAILURE;
    }
  }
...
***************************************************************************
*/
int JpmcdsGenerateUnadjDates(TDate startDate, TDate matDate,
                             TDateInterval *couponInterval, TBoolean stubAtEnd,
                             TBoolean longStub, TDate firstRollDate,
                             TDate lastRollDate, TDateList **dateListPtr,
                             long *stubInfo) {
  ...
  *stubInfo = 0;
  ...
  if (JpmcdsCountDates(startDate, matDate, couponInterval, &numPeriods,
                       &dummyInt) == FAILURE)
    goto done;
  ...
  count = 1;
  do {
    if (JpmcdsMakeDateInterval(couponInterval->prd * direction * count,
                               couponInterval->prd_typ, &interval) == FAILURE) {
      JpmcdsErrMsg("%s: Error generating coupon interval.\n", routine);
      goto done;
    }
    count++;
    if (JpmcdsDtFwdAny(beginDate, &interval, &unadjustedDate) == FAILURE) {
      JpmcdsErrMsg("%s: Error generating unadjusted dates.\n", routine);
      goto done;
    }
  ...
  if (firstRollDate == 0 && lastRollDate == 0) {
    if ((longStub && couponInterval->prd_typ != 'I' &&
         couponInterval->prd_typ != 'E' && couponInterval->prd_typ != 'J') &&
        *stubInfo > 0 && nDates > 2) {
