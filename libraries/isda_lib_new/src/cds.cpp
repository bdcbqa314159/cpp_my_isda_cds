...
***************************************************************************
*/
TContingentLeg *JpmcdsCdsContingentLegMake(TDate startDate, TDate endDate,
                                           double notional,
  ...
  cl = JpmcdsContingentLegMake(startDate, endDate, notional, PROT_PAY_DEF,
                               protectStart);
...
***************************************************************************
*/
int JpmcdsCdsContingentLegPV(TDate today, TDate valueDate, TDate startDate,
                             TDate endDate, double notional, TCurve *discCurve,
                             TCurve *spreadCurve, double recoveryRate,
                             TBoolean protectStart, double *pv) {
  ...
  if (JpmcdsContingentLegPV(cl, today, valueDate, startDate, discCurve,
                            spreadCurve, recoveryRate, pv) != SUCCESS)
    goto done;
...
***************************************************************************
*/
TFeeLeg *JpmcdsCdsFeeLegMake(TDate startDate, TDate endDate,
                             TBoolean payAccOnDefault,
                             TDateInterval *dateInterval, TStubMethod *stubType,
                             double notional, double couponRate,
                             long paymentDcc, long badDayConv, char *calendar,
                             TBoolean protectStart) {
  ...
  if (dateInterval == NULL) dateInterval = &ivl3M;
  ...
  for (i = 0; i < fl->nbDates; ++i) {
    ...
    if (JpmcdsBusinessDay(nextDate, badDayConv, calendar, &nextDateAdj) !=
        SUCCESS)
      goto done;
...
***************************************************************************
*/
int JpmcdsCdsFeeLegPV(TDate today, TDate valueDate, TDate stepinDate,
                      TDate startDate, TDate endDate, TBoolean payAccOnDefault,
                      TDateInterval *dateInterval, TStubMethod *stubType,
                      double notional, double couponRate, long paymentDcc,
                      long badDayConv, char *calendar, TCurve *discCurve,
                      TCurve *spreadCurve, TBoolean protectStart,
                      TBoolean cleanPrice, double *pv) {
  static char routine[] = "JpmcdsCdsFeeLegPV";
  int status = FAILURE;

  TFeeLeg *fl = NULL;

  fl = JpmcdsCdsFeeLegMake(startDate, endDate, payAccOnDefault, dateInterval,
                           stubType, notional, couponRate, paymentDcc,
                           badDayConv, calendar, protectStart);
  if (fl == NULL) goto done;
...
***************************************************************************
*/
EXPORT int JpmcdsCdsPrice(TDate today, TDate settleDate, TDate stepinDate,
                          TDate startDate, TDate endDate, double couponRate,
                          TBoolean payAccOnDefault, TDateInterval *dateInterval,
                          TStubMethod *stubType, long paymentDcc,
                          long badDayConv, char *calendar, TCurve *discCurve,
                          TCurve *spreadCurve, double recoveryRate,
                          TBoolean isPriceClean, double *price) {
  ...
  if (JpmcdsCdsFeeLegPV(
          today, valueDate, stepinDate, startDate, endDate, payAccOnDefault,
          dateInterval, stubType, 1.0, /* notional */
          couponRate, paymentDcc, badDayConv, calendar, discCurve, spreadCurve,
          protectStart, isPriceClean, &feeLegPV) != SUCCESS)
    goto done;

  if (MAX(stepinDate, startDate) <= endDate) {
    if (JpmcdsCdsContingentLegPV(today, valueDate, MAX(stepinDate, startDate),
                                 endDate, 1.0, /* notional */
                                 discCurve, spreadCurve, recoveryRate,
                                 protectStart, &contingentLegPV) != SUCCESS)
      goto done;
  }
...
***************************************************************************
*/
EXPORT int JpmcdsCdsParSpreads(TDate today, TDate stepinDate, TDate startDate,
                               long nbEndDates, TDate *endDates,
                               TBoolean payAccOnDefault,
                               TDateInterval *couponInterval,
                               TStubMethod *stubType, long paymentDcc,
                               long badDayConv, char *calendar,
                               TCurve *discCurve, TCurve *spreadCurve,
                               double recoveryRate, double *parSpread) {
  ...
  for (i = 0; i < nbEndDates; ++i) {
    if (JpmcdsCdsFeeLegPV(today, stepinDate, /* valueDate */
                          stepinDate, startDate, endDates[i], payAccOnDefault,
                          couponInterval, stubType, 1.0, /* notional */
                          1.0,                           /* couponRate */
                          paymentDcc, badDayConv, calendar, discCurve,
                          spreadCurve, protectStart, isPriceClean,
                          &feeLegPV) != SUCCESS)
      goto done;

    if (JpmcdsCdsContingentLegPV(today, stepinDate, MAX(stepinDate, startDate),
                                 endDates[i], 1.0, /* notional */
                                 discCurve, spreadCurve, recoveryRate,
                                 protectStart, &contingentLegPV) != SUCCESS)
      goto done;
...
***************************************************************************
*/
EXPORT TCashFlowList *JpmcdsCdsFeeLegFlows(TDate startDate, TDate endDate,
                                           TDateInterval *dateInterval,
                                           TStubMethod *stubType,
                                           double notional, double couponRate,
                                           long paymentDcc, long badDayConv,
                                           char *calendar) {
  ...
  JpmcdsFeeLegFree(fl);
  if (cfl == NULL) JpmcdsErrMsgFailure(routine);
