...
double JpmcdsZeroRate(TCurve* zeroCurve, TDate date) {
  ...
  if (JpmcdsBinarySearchLong(date, &zeroCurve->fArray[0].fDate, sizeof(TRatePt),
                             zeroCurve->fNumItems, &exact, &lo, &hi) != SUCCESS)
    goto done;

  if (exact >= 0) {
    /* date found in zeroDates */
    if (zcRateCC(zeroCurve, exact, &rate) != SUCCESS) goto done;
  } else if (lo < 0) {
    /* date before start of zeroDates */
    if (zcRateCC(zeroCurve, 0, &rate) != SUCCESS) goto done;
    ...
    if (zeroCurve->fNumItems == 1) {
      ...
      if (zcInterpRate(zeroCurve, date, lo, hi, &rate) != SUCCESS) goto done;
    }
  } else {
...
***************************************************************************
*/
static int zcInterpRate(TCurve* zc, TDate date, long lo, long hi,
                        double* rate) {
  ...
  if (zcRateCC(zc, lo, &z1) != SUCCESS) goto done;

  if (zcRateCC(zc, hi, &z2) != SUCCESS) goto done;
...
***************************************************************************
*/
static int zcRateCC(TCurve* tc, int idx, double* ccRate) {
  int status;

  status = JpmcdsConvertCompoundRate(tc->fArray[idx].fRate, tc->fBasis,
                                     tc->fDayCountConv, JPMCDS_CONTINUOUS_BASIS,
                                     JPMCDS_ACT_365F, ccRate);
  return status;
}
...
int JpmcdsConvertCompoundRate(double inRate, double inBasis,
  ...
  if (IS_EQUAL(inBasis, outBasis)) {
    if (inDayCountConv == outDayCountConv) {
      *outRate = inRate;
    } else if (inDayCountConv == JPMCDS_ACT_365F &&
               outDayCountConv == JPMCDS_ACT_360) {
      *outRate = inRate * 360.0 / 365.0;
    } else if (inDayCountConv == JPMCDS_ACT_360 &&
               outDayCountConv == JPMCDS_ACT_365F) {
      *outRate = inRate * 365.0 / 360.0;
    } else {
      JpmcdsErrMsg(
          "%s: Can only convert between ACT/360 and ACT/365F day count "
          "conventions\n",
          routine);
      goto done;
    }
